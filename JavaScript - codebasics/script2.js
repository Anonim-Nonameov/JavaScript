...
// Посмотрим на определние функции, которая возвращает модуль переданного числа:
...
// Можно ли записать её лаконичнее? Чё-то вроде "return <ответ в зависимости от условия>"?
// Для этого справа от return должно быть выражение, но if - инструкция, а не выражение.

// В JS существует конструкция, которая по своему действию аналогична конструкции
// if-else, но при этом является ВЫРАЖЕНИЕМ. Она называется "тернарный оператор".

// Тернарный оператор - единственный в своем роде (выживший,
// последний из спартанцев..) оператор, требуюший ТРИ операнда:
...
*/
 ...
 // Сокращенный вариант функции abs(), выглядит вот так:

const absShort = (number) => (number >= 0 ? number : -number)

// Обратим внимание на скобки вокруг "тернарника".
// Они вовсе не обязательны, но линтер настоятельно рекомендует их ставить,
// "во избежание неоднозначностей".

// Перепишем начальный вариант getTypeOfSentence() аналогично absShort() -
...
// Я когда то давно писал, что сила выражений (вспомнил мем "в чём сила, брат?")
// очень велика. Уже можно догадаться, что тернарный оператор можно вкладывать в
// другой тернарный оператор.
// "Не делайте этого :) Такой код тяжело и читать, и отлаживать.
// Это очень, очень плохая практика." - (C) - Hexlet.

// Задача из урока:

/*

Реализуйте функцию convertText(), которая принимает на вход строку и, если
первая буква не заглавная, возвращает перевернутый вариант исходной строки.
...
И есть разные подходы к решению этой задачи.
Возможно, вам пригодится метод toUpperCase и возможность получения
смивола из строки, применяя вырезание с индексацией (например, str[0]).
