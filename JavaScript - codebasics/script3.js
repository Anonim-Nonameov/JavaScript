...
// Цикл while идеален для ситуаций, когда количество итераций неизвестно
// заранее, например, при поиске простого числа. Когда количество итераций известно,
// предпочтительнее использовать цикл for.

// Посмотрим реализацию переворота строки через for:
...
// Можно читать так: "цикл с индексом i повторяется пока i < str.length и после
// каждого шага увеличивает i на 1."

// В определении цикла for есть:

// 1) - Начальное значение счётчика. Этот код выполняется ровно один раз перед первой итерацией.
// 2) - Предикат - условние повторения циклов. Выполняется на каждой итерации. Точно так же как и в while.
// 3) - Описание изменение счётчика. Этот код выполняется в конце каждой итерации.
...
Сэмвелл обнаружил, что его сообщения перехватываются в замке "Близнецы" и там
читаются. Из-за этого их атаки перестали быть внезапными. Немного подумав, он разработал
программу, которая бы шифровала сообщения по следующему алгоритму:
...
=> Если число символов нечётное то последний символ остается на своем месте
encrypt('go!'); => 'og!'

Моё решение (опять же подсмотрено):
...
// Што-ш.. курс я за-вер-шил.
// Курс был отличный, много практики, теории.. но мне диииико стыдно за то, что я целых 19
// уроков брал и подсматривал решения.. это был кошмар. Я обязательно пройду его еще раз, но уже БЕЗ
// всяких подсказок, и без конспектов, и пройду его уже самостоятельно! По крайней мере я постараюсь.
// Оценка курсу - 9/10. (9 - за сложность) 