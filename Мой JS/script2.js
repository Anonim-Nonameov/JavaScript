...
сработает и вернёт false, то есть нри одно условие не выполнено, то тут в игру
вступает конструкция else, которая будет выводить в консоль строку "Условие не
выполнено :(". Как-то оно так!

То бишь конструкция else if используется в том случае, когда наше условие не
сработало, но, тем не менее, эта конструкция не такая же, как else - код внутри
фигурных скобок будет работать лишь в том случае, еcли условие внутри круглых
вернуло true, а код внутри фигурных скобок в else отработает тогда, когда условие
в круглых вернёт false. Вкратце, это можно объяснить так: если первое, главное
условие не сработало и вернуло false, мы переходим к следующему условию. Но
тут надо сказать, что условия должны отличаться. Нет, нам никто не запрещает
ставить одинаковые условия везде, где только можно, но.. что толку с этого?
если первое условие не сработает, то не сработают и все остальные, поэтому наши
условия должны отличаться друг от друга. Последнее, что следует добавить, так
это то, что блок (или конструкция, называйте как хотите) else if не является
обязательным и его можно и не использовать, но штука удобная :) - Идём дальше!

Условия: условный оператор "?"

Помимо конструкции if, в JavaScript'е есть оператор "вопросительный знак", и
он полностью удовлетворяет своему названию, это просто знак вопроса! покажу
его использование на примере, но сначала рассмотрим классический пример условия:
...
Итак, объясняю: первым делом мы создаём две переменные, первая называется очень
просто, message, её значение это строка "Привет". Вторая переменная называется
messageEnd, но значения у неё нет, значение ей присваивается при помощи условного
ветвления - если цифра 5 больше, чем 1, то её значение будет строка ", Вася!",
а если нет, то её значением будет строка ", Оля!". Но, поскольку цифра 5 больше,
чем 1, её значением будет строка с именем "Вася". Далее, после услов. ветвления
мы складываем строку, находящуюся в переменной message со строкой, которая
находится в переменной messageEnd и выводим переменную mesage в консоль, в
результате получаем строку "Привет, Вася!". Это был самый классический пример
использования условного ветвления, но сейчас я покажу, как при помощи оператора
"вопросительный знак" переписать это классическое условное ветвление:
...
А теперь я объясню, что значит код выше: в начале всё просто, происходит то же
самое, что и в прошлом примере, но значение в переменной messageEnd всё таки
есть. Что же это за значение? ей присвоено условие, и если оно выдаст true,
то в messageEnd будет присвоено одно значение, если же выдаст false - другое
значение. Условие следующее: "если 5 больше, чем 1, то значение будет ", Вася!",
а если нет, то значение будет ", Оля!". 5 больше, чем 1, соответственно значение
этой переменной равно строке ", Вася!". Выглядит всё следующим образом - после
оператора присвоения идёт само условие в круглых скобках, а после него идёт
совершенно другой оператор, оператор "вопросительный" знак. Здесь он играет
роль конструкции if и всё то, что идёт после него - выполняется лишь в том случае,
если условие вернуло true. После оператора "?" у нас идёт строка ", Вася!", и из
этого следует, что если условие вернёт нам true, то в переменную будет записана
как раз эта же строка. После этой строки идёт другой оператор, оператор "двоеточие"
и он выполняет роль конструкции else. Здесь то же самое, что и с оператором "?" -
всё то, что идёт после ":", будет выполнено лишь в том случае, если условие выдало
нам false, и в данном случае значение переменной будет строка ", Оля!", но я
повторяю, что это случится лишь в том случае, если условие вернёт false. А далее
мы берём и складываем строки в двух переменных - message и messageEnd, по итогу
у нас выходит одна строка "Привет, Вася!", её мы и выводим в консоль. Следует
также отметить, что круглые скобки можно убрать и оставить лишь условие, так
как оператор "?" имеет крайне низкий приоритет, поэтому сначала выполнится
условие и только потом условный оператор. И последнее, что следует сказать об
этом операторе, это то, что он - тернарный. Это единственный оператор, у которого
может быть сразу 3 операнда.

Условный оператор "?" - сложные операции

С помощью этого условного оператора мы можем делать очень интересные и сложные
вещи. Мы можем создавать более сложные конструкции, включащие в себя блоки else
и else if. Посмотрим на пример ниже, "классическое" применение конструкции доп.
условия else if:
...
В примере выше, мы берём и создаём переменную message, а в качесте значения
присваиваем строку "Привет". Далее создается переменная messageEnd, но никакого
знчения мы ей не присваиваем, это за нас сделает условное ветвление. Если,
допустим, 5 больше 50-ти, то значение переменной messageEnd будет ", Вася!",
но это не так, 5 МЕНЬШЕ 50-ти, поэтому мы идём дальше, к следующему условию.
Если 5 больше 30-ти, то значение messageEnd будет равно той же строке, но
вместо имени "Вася" будет "Оля". Это условие вернёт false, идём дальше - если
5 больше 10-ти, то значение переменной messageEnd будет равно всё также той
же самой строке, только вместо имени "Оля" будет "Миша". Неверно, дальше - в
случае, если 5 больше чем 1, то значение переменной будет опять таки равно
той же строке, но вместо имени "Миша" будет имя "Андрей". Это условие истинно,
поскольку 5 действительно больше чем 1, поэтому значение переменной теперь равно
строке ", Андрей!". Но если бы и это условие вернуло false, то сработала бы
конструкия else, которая присвоила переменной значение строки ", Иннокентий!".
Далее мы берём и складываем строки, по итогу получается строка "Привет, Андрей!",
её мы выводим в консоль и там же получаем.

Теперь взглянем на тот же пример, но с использованию условного оператора "?":
...
Пример выше повторяет тот же код, что и предыдущий, "классический". Разница лишь в
том, что последняя реализация намного менее читабельна, и крайне рекомендуется
использовать классическую реализацию.

По той же самой причине нельзя полностью заменять конструкцию if/else этим
условным оператором, потому что полностью (на 70%) теряется читабельность кода.
Достаточно посмотреть на пример кода снизу:

5 > 10 ? console.log("Привет, Вася!") :
    console.log("Привет, Оля!");
=> "Привет, Оля!"

Выше, мы полностью заменили конструкцию if и использовали лишь условные операторы,
и прочитать этот код можно следующим образом: "если 5 больше 10-ти, то в консоль
выводим строку "Привет, Вася!", если же нет - в консоль выводим строку "Привет,
Оля!". Последнюю строку мы и получаем в консоли, ибо 5 не больше 10-ти, оно
меньше. А вот как выглядел бы этот код, если бы мы использовали конструкцию if
с блоком else:
...
Как мы видим, предыдущий блок кода и тот, что выше - сильно внешне отличаются,
хоть и выполняют они одну и ту же работу. Опять же, делать так крайне не реко-
-мендуется, и использовать условный оператор надо в том случае, когда нам
нужно вернуть то или иное значение, как в примере ниже:

let message = 5 > 10 ? "Привет, Вася!" : "Привет, Оля!";
console.log(message);
=> "Привет, Оля!"

Код выше создаёт переменную message и присваивает ей то или иное значение, в
зависимости от того, что вернёт условие. Если true - значением будет "Привет,
Вася!", если нет, то "Привет, Оля!". Далее мы выводим в консоль значение
переменной и получаем строку "Привет, Оля!", поскольку условие вернуло false.

А на этом всё! - остальное будет позже :)
*/