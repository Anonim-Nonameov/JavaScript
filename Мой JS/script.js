...
с объектами, а во вторых - "length.2" как-бы укоротил наш массив до 2 элементов,
...
В случае, если элементы массива являются объектами, нам на помощь придут методы
find и findIndex. Собственно, эти методы обеспечивают поиск в массиве объектов С
...
Проверка массива - Array.isArray(): шпионы среди нас...

И так.. настал тот момент, когда мы будем ПРОВЕРЯТЬ наш массив на верность нам.
Напоминаю, что массивы - не отдельный тип данных, они основаны НА ОБЪЕКТАХ!
Взгляните на пример:
...
В обоих случаях мы получаем объекты. Это вполне логично, массив же родственник
объекта. Но тут встает вопрос - как нам узнать, где массив а где нет? и на этот
вопрос я бы ответил, что можно посмотреть на скобки (если квадратные - значит
массив), но что если мы получаем данные с сервера и не знаем, объект ли к нам
пришел или массив, собственной персоной? в таком случае, для проверки мы можем
использовать конструкцию if-else и Array.isArray:
...
Что мы сделали в коде выше? ничего особенного - мы просто проверили наш массив
на то, является ли он массивом или нет, как бы странно это не звучало. После
этой проверки мы вводим в консоль запись "Это массив!", если то, что мы передали
оказалось массивом, конечно же. В ином случае мы выводим надпись "Это не массив!",
но её мы не получили, так как наш массив оказался... ну, вы поняли. А что будет,
если мы проверим ОБЪЕКТ? по сути, он должен пройти проверку, потому что сами
массивы основаны на них. Но всё впереди, а пока что проверяем:
...
Как мы видим, объект не прошел проверку - это не массив. И на самом деле это
не очень то и странно, эта команда проверяет переданный элемент на то, является
ли он массивом или же нет. Объект - не массив, вот и не прошел проверку.

Перебор элементов: так лень делать это вручную...

Идем дальше, к методам для перебора массива. Под перебором массива подразумевается
ПОЛНЫЙ перебор его элементов. Во время перебора мы можем делать с элементами что
угодно, или самое простое действие - выводить их в консоль. Для первого способа
перебора массива нам не нужен метод, а цикл for:
...
Код выше объясняется так: создаем массив из трех элементов, после него цикл и
итератор внутри цикла. Цикл будет продолжаться до тех пор, пока не кончится
весь массив (строка "i < arr.length"), и каждую итерацию наш итератор будет
постепенно увеличиваться на единицу. А уже в самом теле цикла мы просто берем
и выводить каждый отдельный элемент массива по индексу, равному итератору - если
итератор равен нулю, то в консоль будет вызван первый элемент массива, если же
он равен единице - в консоль будет вызван второй элемент и так далее.

Это всё хорошо, но мы можем использовать специальный цикл for..of. В прошлом
примере мы убедились, что мы можем получать и значение и ключ массива, но если
нам достаточно выводить только значения массива, то мы можем использовать этот
цикл. То есть да - он тупо выводит значения массива и всё, ничего больше.
Посмотрим на пример использования метода:
...
Синтаксис у этого цикла очень отличается от прошлого цикла for, например: в кру-
-глых скобках мы объявлем переменную, которая будет содержать каждое ЗНАЧЕНИЕ
массива. После этого пишем of, а затем уже название той переменной, которая со-
-держит в себе массив. Внутри фигурных скобок можно выбрать, что же мы будем
делать с каждым элементом массива. А нам достаточно просто выводить в консоль
эти элементы, это мы и сделали - получили 3 элемента массива. Идем дальше!

Если раньше мы использовали циклы для перебора массива, то сейчас мы поговорим
о самом настоящем МЕТОДЕ для перебора массива - forEach! и он может не просто
перебирать массив - он еще может выполнять свою собственную ФУНКЦИЮ (которую
мы можем настроить, кстати говоря) для каждого элемента массива. Посмотрим
на примере, так будет легче объяснить работу метода:
...
=> "Ваня находится 0 позиции в Ваня,Ишван,Оля"
=> "Иштван находится 1 позиции в Ваня,Ишван,Оля"
=> "Оля находится 2 позиции в Ваня,Ишван,Оля"

Объясняю: мы создаем массивы и определяем в него 3 элемента, таким образом те-
-перь у нас есть массив из 3 элементов. Далее мы обращаемся к этому массиву и
применяем на нём метод forEach. В качестве аргументов передается функция, а
в качестве аргументов самой функции передаются следующие 3 уже знакомые нам
параметра - item, текущий элемент массива; index, позиция текущего элемента;
array, сам массив (точнее его скелет,  элементы). В теле функции мы можем
делать с этими элементами ВСЁ, что захотим. В нашем случае мы просто выводим
в консоль строки, количество которых равно количесту элементов в массиве. В
каждой такой строке мы указываем элемент, его позицию и сам массив в следующем
формате: "(элемент) находится на (индекс элемента) позиции в (массив)". После
этого в консоль мы получаем три строки, в каждой из которых свой элемент, своя
позициия, но один и тот же массив. И всё это прекрасно, но запись можно чутка
укоротить, используя стрелочную функцию:
...
Ну вот, кода стало чуть меньше, а результат не изменился! Чудненько :)
И вы сейчас могли подумать, что это всё - но нет. В аргументы методы forEach
мы можем передать кастомную функцию, которая и должна будет выполниться:
...
На этом с методом forEach мы покончили, идем дальше!

Методы reduce и reduceRight - мощь массивов!

1) reduce

Итак, теперь мы знаем, что если нам нужно перебрать массив - используем метод
forEach (ну по желанию, по желанию), for или for..of. Если нам нужно перебрать
массив и вернуть данные для каждого элемента - используем map. Это хорошо.
Методы arr.reduce и arr.reduceRight похожи на эти методы, но отличаются они
тем, что они намного сложнее и используются они для вычисления ОДНОГО значения
на основе всего массива. Посмотрим на синтаксис метода reduce, reduceRight мы
пока что затрагивать не станем:

let value = arr.reduce(function(previousValue, item, index, array) {
    ... (типа код)
}, [initial]);

К знакомым нам item, index и array добавились previousValue и initial. Но тут
у вас возникнет вопрос: "что же это такое?", и правильно, что же это такое?
сейчас мы это и узнаем, а начнем с previousValue. previousValue - результат
предыдущего вызова той функции, что является первым аргументом метода reduce,
и он равен начальному значению при первом вызове функции (если оно, начальное
значение, был передано). Начальным значением здесь обозначается параметр initial,
который является ВТОРЫМ аргументом метода reduce после функции. Про item мы уже
всё, что нам нужно, знаем - это элемент массива, index - индекс элемента, а вот
array - сам наш массив. Функция здесь применяется по очереди ко всем элементам
массива и как-бы "переносит" свой результат на следующий вызов. И тут ОПЯТЬ у
вас может возникнуть вопрос - что значит "переносит"? разберемся на примере!
...
Объясняю код выше: создается массив чисел с названием arr, далее создается другой
массив, у которого название reduceValue. В качестве значения мы указываем просто
указываем результат работы метода reduce с массивом arr, ничего интересного. Но
интересно то, что проиходит внутри! первым аргументом метода мы указываем функцию
(как и положено), а вторым аргументом указываем цифру 0 как начальное значение.
В качестве аргументов уже самой функции передаем 4 параметра: previousValue, item,
index & array. Внутри функции мы СКЛАДЫВАЕМ текущий элемент массива с предыдущим
значением, с previousValue. После этого мы просто возвращаем сумму, а дальше мы
просто выводим в консоль значение reduceValue. Выводится нам число 10. Но почему
нам вывело 10? разберем всё по шагам:

-----------------------------------------------------------------------------

Шаг №1
- previousValue равен нулю (это начальное значение, как мы и указали)
- item равен единице (первый элемент массива)
- их сумма равна единице (0 + 1 = 1)

Шаг №2
- previousValue равен единице (так как предыдущая сумма была равна 1)
- item равен двойке (второй элемент массива)
- их сумма равна трём (1 + 2 = 3)

Шаг №3
- previousValue равен трём (предыдущая сумма)
- item равен трём (третий элемент массива)
- их сумма равна шести (3 + 3 = 6)

Шаг №4 (последний)
- previousValue равен шести (предыдущая сумма)
- item равен четырём (четвертый элемент массива)
- их сумма равна десяти (6 + 4 = 10)

-----------------------------------------------------------------------------

Ну вот и всё, ничего сложного! если разбирать всё пошагово, то всё становится
предельно ясным: на первом шагу мы сложили текущий элемент массива (то есть
единицу) с начальным значением, а это был нолик. Ноль + единица равно единица.
На втором шагу мы проделали то же самое, но вместо начального значения мы
брали ПРОШЛОЕ значение, прошлую сумму. Вот так складывая и складывая, потихоньку
мы добрались до финала, и там уже нас ждало число десять -  всё просто! :)

Но что, если мы не будем указывать начальное значение? в таком случае, начальным
значением становится первый элемент массива, а работа метода начинается со второго
элемента. Посмотрим на пример:
...
Объясняю, что и где сейчас произошло: мы создаем массив из трёх элементов-строк и
помещаем его в переменную arr, далее мы создаем переменную reduceValue и в каче-
-тве значения присваиваем результат работы метода reduce. Внутри него создаём
функцию, в которую передаём те же 4 аргумента - предыдущее значение, элемент,
его индекс и сам массив. Внутри тела функции мы выполянем следующее: выводим
в консоль начальное значение, но мы не указали его явно, поэтому им будет первый
элемент массива, то есть "Ваня". Далее мы выводим элемент массива, но метод
начнет свою работу именно со второго элемента массива, а не с первого, потому что
первый уже занят - он начальное значение. Следовательно, в консоль выводится наш
"Иштван". После этого мы возвращаем одновременно и элемент, и предыдущее значение,
в результате получаем строку "Иштван, Ваня". После этого мы выводим в консоль
последний оставшийся элемент и получаем "Олю". Закрыв все скобки в теле функции
и закончив с ней работу, мы выводим в консоль результат работы метода после строки
"Пользователи:", а именно - "Оля, Иштван, Ваня", где Оля с Иштваном это элементы,
а Ваня - предыдущее значение. Идём дальше!

2) reduceRight

Метод arr.reduceRight работаем аналогично, но проходит он по массиву СПРАВА и НА-
-ЛЕВО, и поэтому мы даже не будем смотреть на пример - всё и так предельно ясно!)

Файл стал слишком большим, создал новый :)
*/