...
// Итак, тот цикл который мы изучили вчера - называется "цикл с пост-условием", это такой
// цикл который сначала выполняется, а потом соблюдает условия. Т.е - сначала делает,
// а потом думает (кто знает, может быть еще и сожалеет? :D) :)
// Но есть и другой, противоположный вид - "цикл с пред-условием". Это тот же самый цикл while,
// только без ключ. слова "do", и он является последним, третьим ццклом. Синтаксис его прост:

/*

while (условие) {
	код, код, МНОГО КОДАААА!!!
}

*/

// Т.е, в этом цикле мы сначала что-то проверяем, а затем уже что-то делаем. И опять же - здесь
// do не нужен, а фиг. скобки ставятся сразу после круглых.
// А теперь испробуем этот цикл:

/* let */ i = 0; // обнуляем наш итератор, делая его значение равным нулю
while (i < 10) {
	console.log(i) // => 0 на первой итерации
	i++; // 10 на последней итерации
}

// => 0...9

// Итак, разберем логику нашего кода:
// В самом начале инициализируется переменная i (должна была инициализироваться, но я то просто
// поменял значение, а не создал заного), её значение равно нулю. Она попадает в условие, и наш
// видит и переменную и условие, то что оно выполняется. Так как переменная равна условию
// (i < 10), цикл "даёт добро" и позволяет выполниться коду внутри фиг. скобок.
// А внутри фиг. скобок мы выводим текущее значение итератора (переменная i), а затем
// на каждой итерации увеличиваем её на один. На первой итерации наша переменная будет
// равна нулю (ну всё правильно - мы ж обнулили её :>), а на след. итерации - равна единице.
// После этого, переменная будет постоянно увеличиваться до 10. На последней она как раз
// и достигнет десятки, и провериться условие (всё как обычно), и тут: несовпадение с условием!!
// 10 не меньше 10-ти, они РАВНЫ (==), поэтому цикл останавливается и код внутри фигурных
// скобок не выполняется, а последним выведенным значением i остается цифра девять (9).

// P.S - переписал всё то, что было сверху. Своего рода remaster текста :D

// Ну вот, мы изучили последние оставшиеся циклы. Они не очень популярны как for,
// да и к тому же у них нет возможности создавать переменную внутри себя, как в
// цикле for, и используют их намного-намного реже. Но всё это не повод их не изучать!

// А сейчас у нас домашнее задание!
// Мы когда-то написали код, который (при заходе юзера на страницу) задает вопросы, а затем
// сохраняет ответы в массив и выводит (уже оттуда, из массива) на экран (в браузер).
// Необходимо переписать весь этот код так, чтобы он работал на "while (;) {...}" и
// на "do {...} while (;)", т.е - убрать for и заменить его сначала на "do {...} while(;)",
// а затем на "while (;) {...}".

// Ну это не сложно, даже не серьезно.
// Первым делом создадим код с do {...} while(;) -

/*

// - Этап инициализации итератора, придание ему значения "0"
let i = 0; 

// - Этап создания массива questions, в котором будут храниться вопросы
let questions = [
	"Как ваше имя?",
	"Как ваша фамилия?",
	"Сколько вам лет?",
	"Сколько перьев у курицы?",
	"Как долго космонавт может не ходить в туалет?"
]

// - Создание массива answers, будет заполняться ответами на вопросы из questions
let answers = [];

// - Этап создания самого цикла:

1. - В первую очередь мы должны сделать так, чтобы в массив answers поэтапно вписывались ответы
на вопросы: такой код есть выше в цикле for, там же его подробное объяснение.
2. - Так как у нас нет встроенного итератора, увеличивать его мы будем внутри цикла.
3 - Условие то же самое, что и в коде с for выше.
...
// - Все этапы будут пропущены, ибо код тот же самый. Просто оставлю готовый код
...
// А на этом всё! В следующем уроке поговорим об "условиях".