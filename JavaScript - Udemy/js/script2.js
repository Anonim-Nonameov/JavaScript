...
// Функции: директива "return"

// Итак, несколько дней назад мы с вами начали изучение функций, чуть позже
// изучили как работают аргументы, и теперь у нас есть базовые значния о
// функциях и мы можем с легкостью создать простую функцию! но функции не
// ограничиваются тем, что мы изучили - мы еще не выжали все соки из функций,
// поэтому с сегодняшнего дня мы начнем идти "вглубь" функций и изучим их на 100%.
// Начнем мы с того, что наш калькулятор... хорош, по крайней мере как для новичка.
// Но он может вернуть значение не только в консоль, а просто "вернуть". Что это
// значит? - если мы уберем (у кода выше) код внутри фиг. скобок, оставив только
// выражение "a + b", и перед ним поставив специальную команду return - наша
// функция будет просто будет считать сумму и возвращать её из функций. Функция
// сможет вернуть только ответ, она не сможет самостоятельно вернуть его в
// консоль или выводить на экран пользователя - она просто возвращает число.
// Что делать с этим числом - мы решаем сами, дальше функция ничего сделать
// не сможет. Например, при вызове такой функции (с return) с некоторыми
// аргументами, например 2 и 7, наша функция вернет 9. Девятка у нас будет
// просто валяться в функции, ничего не делая! но мы можем сделать вот что:

// Для начала создадим саму функцию, на этот раз другую - с return:

function calcReturn(a, b) {
	return a + b;
};

// А теперь пришла пора интересностей:

console.log(calcReturn(2, 7));
// => 9

// Итак.. что мы с вами (ну, точнее я) сделали? - мы взяли и вывели в консоль
// сумму, получившуюся в результате сложения чисел 2 и 7. В тот момент, когда
// мы написали "calcReturn(2,7)", наша функция "вернула" значение: 9, а console.log()
// вывел его в консоль!

// Функции: Function Declaration/Expression

// есть еще одна очень важная информация, которую мы должны знать и помнить - это
// классификация функций. Дело в том, что мы можем записать функции в виде "Function
// Expression" и "Function Declaration". Что это значит? ну, если говорить "чело-
// вечьим" (:D) языком, то мы либо функцию объявляем самым обычным способом, а
// это Function Declaration, либо записываем её в формате функционального выражения.
// Например, возьмем те три функции, ко- торые мы объявили ранее - это и есть Function
...
3) Функция-калькулятор - версия с return:
...
// Вся особенность функций из "Function Declaration" в том, что мы можем
// использовать их ДО объявления! и я не шучу, вот, посмотрите сами:

realnayaMistika("Vasya");
// => "Hello Vasya!)"

function realnayaMistika(name) {
	console.log('Hello ' + name + "!)");
};

// Как мы видим, нам действительно выдало фразу-приветствие! давайте разберемся,
// как работает этот механизм, позволяющий использовать функции до их объявления.
// Дело в том, что когда наш скрипт запускается, index.html доходит до тега
// <script>, подключает этот файл и браузер первым делом быстро пробегается
// по нашему JS коду. Помните, как у нас всё происходило с переменным, объяв-
// ленными через var? если нет, то я напомню, что браузер находил их (т.е пере-
// менные, созданные через var) и создавал еще до того, как код запускался.
// С этими функциями происходит всё то же самое! сначала браузер эти функции
// создает, а потом уже запускает наш скрипт. И здесь получается такая ситуация,
// что мы можем использовать функции до их создания в коде. Ну а что - теоре-
// тически наша функция уже существует, по крайней мере "её призрак" :D
// Но это поведение мы можем поменять, сделать нечто более интересное. Возьмем
// функцию calc, которую мы с вами когда-то создавали - что если сделать.. вот так?

let mistikaRealnaya = function(a, b) {
	return a + b;
};

console.log(mistikaRealnaya(12, 47));
// => 59

// Что мы с вами сделали? мы создали переменную с неким названием, а в качестве
// значения мы поставили функцию, которая принимает два аргумента и затем скла-
// дывает их, "возвращая" результат. Но нас с вами должно интересовать то, что
// происходит непосредственно ПОСЛЕ создания переменной - мы выводим в консоль
// сумму сложения чисел 12 и 47 используя функцию, название которой - наша пе-
// ременная! вся соль в том, что наша переменная фактически стала функцией, ко-
// торую мы можем использовать в любое время, и имя этой функции - название на-
// шей переменной. Такую функцию мы можем использовать сколько угодно раз, это
// самая обычная функция. Но её отличие от прошлой в том, что это - Function
// Expression. Мы не можем использовать такую функцию ДО её создания, как прошлую.
// Всё дело в том, что функциональные выражения (expression означает выражение)
// создаются тогда, когда работающий код доходит до него. Это значит, что пока
// скрипт не дойдет до того места, где наша функция создается - она работать
// не будет. А это значит, что вызвать такую функцию до её создания мы НЕ сможем.
// Чаще всего в JavaScript используют формат "Function Declaration", по уже по-
// нятным причинам, так что и нам с вами необходимо использовать такой формат,
// чтобы не возникало ошибок (функция типа "Expression" РА.БО.ТАТЬ НЕ. БУ. ДЕТ).

// Функции: области видимости

// A.S: итак.. давненько я не делал обновлений - целую неделю никаких обнов, вообще
// никаких! а всё дело в том, что доступа к компу у меня не было.. возникили кое
// какие проблемы, которые я уже решил. На самом деле, доступ появился в пятницу
// ночью, а ночью делать 2 урока по 75 минут я бы не смог. В субботу же я решил
// не делать уроков, так как в воскресенье у меня должен быть выходной! какой
// смысл делать уроки, если на следующий день опять отдых? и я решил всю неделю
// завершить, не делая никаих уроков и, соответственно, обновлений. По этой причине
// на этой неделе будет x2 консплекта и не будет никаких выходных :) TFA!

// Теперь мы поговорим об еще одной важной теме в JS - области видимости функций.
// Что такое область видимости? сейчас узнаем! а пока что давайте создадим пере-
// менную count, а начальным значением у неё будет циферка 8. Наша переменная
// должна быть глобальной, поэтому я создам её в самом начале нашего файлика,
// но оставлю здесь вот такой комментарий, чтобы не было путаниц:

// let count = 8;

// Ничего интересного, простая переменная с простейшим значением внутри неё.
// И, естественно, мы можем вызать значение переменной в консоль, с этим никаких
// проблем не будет (понятное дело):

console.log(count);
// => 8

// Но что если мы поместим точно такую же переменную ВНУТРЬ ФУНКЦИИ?
// А после того как поместим её внутрь функции, будем увеличивать её значение
// инкрементом и затем выведем в консоль её значение! функцией, внутри которой
// будет храниться наша переменная, будет та самая функция humanSayHello:

function humanSayHelloWV(user) {
	let count = 10;
	count++;
	console.log(count);
	console.log('Hello ' + user + '!');
...
// A.S: WV означает "with variable"
// Отлично, а теперь нам необходимо как обычно это бывает вызвать функцию.
// В этом ничего трудного нет, но трудно будет понять, что же нам выведется
// в консоль! подумайте над этим, затем выполните такой код. Что будет выведено
// нам в консоль - я не скажу:

humanSayHelloWV();
// => ?

// Отлично! а теперь выведем в консоль значение переменной count, но изменим
// его на 5 (потому что у лектора она изначально равнялась 5):

count = 5;
console.log(count);
// => 5

// Результат неожиданный, так что давайте разбираться, осматривая каждую строку.
// Начнем с того, что мы создали переменную count, которая является по сути
// глобальной, так как она находится выше всех остальных переменных и функций и
// доступна везде в коде. Затем, мы создали функцию, внутри которой есть собствен-
// ная ОБЛАСТЬ ВИДИМОСТИ (которую дальше я буду сокращать до ОВ). Т.е, всё что
// ограничено фигурными скобками - ОВ нашей функции. Если мы здесь (имеется ввиду
// внутри фиг. скобок) создадим еще какую нибудь коробочку с информацией (то бишь
// переменную), и даже назовем её как и нашу глобальную переменную, то она будет
// храниться только внутри этой функции. И так мы поступили на строке №675, где
// создали переменную с таким же названием как и у глоб. перем. На след. строке
// мы увеличили значение нашей переменной на 1, так что оно теперь равнялось не
// 10 а 11. И уже после этого мы пытаемся вывести в консоль значение переменной
// count, и тут же начинается самое интересное! когда браузер (ну, интерпретатор)
// смотрит на наш код и видит как мы пытаемся вывести в консоль значение переменной
// count, он первым делом пытается найти такую переменную внутри функции, то есть
// внутри её локального окружения. Тооо ееесть он сначала ищет локальную перемен-
// ную. Если вдруг он её нашел (а он её реально нашел) то он выводит её в консоль,
// и именно поэтому мы увидели весьма неожиданный результат в консоли. На след.
// строке у нас всё то же самое что и у обычной функции humanSayHello, ничего
// нового и необычного. Ну, с функцией мы покончили, а что идет дальше?
// А дальше ничего интересного: простой вызов функции. Но нас это не итересует,
// нас интересует то, что идет после вызова функции! а после у нас идет вызов уже
// другой функции, которая выводит в консоль значение переменной count. Но эта
// команда написана уже как бы в глобальной области видимости, т.е она не нахо-
// дится внутри функции. Поэтому, первое что она делает, она пытается найти где-то
// рядом переменную count (правда она не рядом, но и так сойдет :DDD), и таки нахо-
// дит её, выводя её значение в консоль. Именно поэтому у нас такой странный и,
// повторюсь, неожиданный результат.

// Можно еще поэкспериментировать, для того чтобы понять как всё это (ОВ) работает.
// Для начала создадим НОВУЮ функцию, у которой создание функции и увеличение её
// значения на 1 - пропускается (т.е, мы оставим лишь вывод в консоль count):

function humanSayHelloWHTVI(user) {
	console.log(count);
	console.log('Hello ' + user + '!');
}

// A.S: WHTVI означает "WitHouT Variable Initialization"
// Круто, а теперь мы проделаем те же самые действия, что и ранее: используем
// нашу функцию и выведем в консоль значение переменной count, но я добавлю также
// и "пробел", чтобы в консоли не было путаницы. Опять же - результат использо-
// вания функции я скрою, сами узнаете (так даже интересней):
...
// Итак.. результат в консоли опять же неожиданный, поэтому давайте опять разби-
// раться, что же произошло и почему. При запуске функции, функция пытается найти
// переменную count, внутри её ОВ. Эту переменную она конечно не находит, поэтому
// функция "вылазеет" наружу, в глобальную ОВ, и начинает искать эту переменную
// там же. В конечном итоге функция видит такую переменную и использует её, т.е
// выводит её значение в консоль. Именно поэтому результат выполнения функции
// такой неожиданный: сначала мы видим тот самый скрытый результат, а затем
// пятерку! в общих чертах всё так и работает: функция либо ищет внутри себя,
// либо снаружи - в глобальной ОВ. Ну, с этим мы наконец-то покончили!

// А теперь давайте закреплять всё это на практике. Сейчас мы будем создавать
// абсолютно НОВУЮ программу, но лектор её не завершит до конца - этим будем
// заниматься МЫ! мы должны будем завершить эту программу, используя все
// полученные нами знания. Давайте же начнем создавать нашу программу :)

// Первым делом мы создадим функцию myFirstApp, которая будет принимать два
// аргумента: name и age. Внутри фигурных скобок будет простейшая конструкция,
// начинающаяся с команды alert. Внутри alert'а будет сообщение, но не простое!
// я хочу сформировать его при помощи имени, которая будет передаваться в программу
// через аргумент name. И здесь я хочу рассказать вам о кое чем ооооочень интересном,
// о чем-то что пришло нам из ES6 стандарта - о интерполяции. Помните, мы как-то
// говорили о "нескольких видах кавычек"? так вот, интерполяция использует именно
// косую (или же обратную) кавычку, которую чаще всего называют "бэктик". Находится
// она на клавише "Ё", а как добавлять "второстепенные" символы с клавиатуры - вы
// скорее всего знаете, я думаю :) но для чего мы будем использовать бэктики? дело
// в том, что они помогут нам использовать переменные ПРЯМО ВНУТРИ НИХ! помните
// как мы недавно складывали строки через конкатенацию? здесь будет похожая кон-
// струкция, которая и называется "интерполяция". Вкратце, это упрощенная версия
// конкатенации, созданная для того, чтобы облегчить нам жизнь (не смог подобрать
// других слов, это больше всего подходит :D). А если без шуток - с интерполяцией
// наш код будет выглядеть намного красивее и компактнее! Но не будем забегать так
// сильно вперед, лучше показать как будет выглядеть наш код! выглядит всё (по
// крайней мере на данном этапе) так:
...
// Так вот, я писал что мы сможем добавлять внутрь них переменные.. вы наверняка
// подумали: "и что же в этом удивительного? с конкатенацией тоже самое!", и если
// вы подумали именно так - вы абсолютно правы! но дело в том, что каждое слово
// в конкатенации использует свою строку! с интерполяцией мы можем использовать
// только 1 вид кавычек, и писать внутри них как текст, так и переменные! добав-
// ляются они просто: [ДАННЫЕ]...[УДАЛЕНЫ]
// Эээ, куда так торопимся?! для начала надо добавить текст в наш alert! внутри
// бэктиков мы пишем: "Привет, меня зовут ..." И СРАЗУ ЖЕ ОСТАНАВЛИВАЕМСЯ! вместо
// точек мы ставим значок доллара - $ (ох, всё так по богатому!) и сразу после них,
// всмысле без пробела, ставим фигурные скобки. Внутри них пишем имя переменной и...
// и что дальше? а дальше ничего! после имени переменной идёт закрывающая фигурная
// скобка, и что-то еще. И этим чем-то еще будет текст, а именно такая простая
// надпись: "и это моя первая программа!". вот как всё это будет выглядеть в коде:
...
// Как мы видим, код сразу же начал казаться более привлекательным! нам больше
// не нужно разрывать строку, всё пишется в одной единственной строке.
// На этом этапе мы покончили с alert'ом, идем дальше! если вы еще помните, я
// говорил (а точнее писал), что функции могут содержать внутри себя и другие
// функции! поэтому мы создадим еще одну функцию, называться она будет showSkills
// и она будет идти после alert'а. Никаких аргументов принимать она не будет,
// поэтому идем дальше - к фигурным скобкам. Раскрываем их и создаем массив с
// названием "skills" (это будут те навыки, которыми вы владеете). Напомню, что
// массив данных служит нам для того, чтобы мы могли поместить внутрь него какую-
// то информацию по порядку. Например, то что мы владем HTML, CSS, JS.
// Еще раз напомню, что у нас вся эта информация будет записана в виде строк (я
// имею ввиду наши навыки), если нет - будет ошибка! а теперь покажу код:
...
// Отлично! но есть одно но - массив можно заполнять как угодно! но я думаю,
// что этих навыков будет предостаточно :) всё, с массивом также закончили.
// Но впереди осталась главная задача, которую нам предстоит решить: вывести
// все эти навыки на экран, по-очередно, чтобы у нас вывелось три подобных
// сообщения: "Я владею: HTML", затем "Я владею: (другой навык)" и еще одно
// такое же сообщение, и да - не важно куда будут выводиться наши сообщения,
// можно в консоль а можно в виде alert'а. Самое главное, чтобы мы не вруную
// всё это делали, т.е чтобы не писали три отдельных console.log'а или три
// отдельных alert'а. Вспомните, как мы уже оптимизировали код: представьте,
// что если наш массив разростется на много-много навыков? будет ли ваш код
// продолжать выводить элементы из массива? подобную задачу мы уже когда-то
// решали, так что мы с ней справимся (надеюсь). Кстати, не забывайте о том,
// что созданные функции необходимо ВЫЗЫВАТЬ! этим мы тоже займемся.

// А мы идем дальше - создаем функцию checkAge, которая не будет принимать
// в себя никаких аргументов (как и showSkills). Внутри неё мы будем проверять
// возраст пользователя (который будет передаваться в age), и если, допустим,
// нашему юзеру 18 лет - мы выводим (опять же не важно куда, в консоль или
// на страницу alert'om) следующее сообщение: "У тебя отличные шансы стать
// Frontend dev'ом!". Если же юзеру меньше 18 лет, то мы будем выводить след.
// сообщение: "Я удивлён, что ты так рано задумался о изучении Frontend'а..."
// (хотя по сути это сказки, во фронт зайти можно в любом возрасте). И опять
// же - сообщения могут быть абсолютно разными! вместо сообщения, которое
// должно выводиться если юзеру нет 18-ти лет, мы можем подставить совершенно
// другое! всё абсолютно на ваше усмотрение, не хотите так - будет по другому.
// А вот и пример кода, который должен у нас получиться:
...
function myFirstApp(name, age) {
	...
	function showSkills() {
		let skills = ["HTML", "CSS", "JS"];
		(дальше должны выводиться навыки)
	}

	function checkAge() {
		(тут должно быть условие, которое проверит, равен ли возраст 18-ти:

		1) если да - выводим сообщение "У тебя отличные шансы стать Frontend dev'ом!",
		2) если меньше 18-ти - выводим сообщение "Я удивлён, что ты так рано задумался
		о изучении Frontend'а...")
	}
}

*/

// Не останавливаемся - движем дальше! создаем функцию calcPow, которая будет
// принимать 1 аргумент - "num", то бишь число. Внутри фигурных скобок мы также
// должны создать функцию, которая будет искать квадрат этого числа. Как найти
// квадрат числа, я думаю, вы знаете, не так уж это и сложно (если вы учились) :)
// А на этом создание функции завершено, осталось лишь её дополнить, а так.. усо!
// Что нам нужно сделать и дополнить? ниже предоставлен список всего необходимого.
// A.S: выводить информацию мы будем непосредственно в браузер, юзая alert.

// 1) - сделать так, чтобы в браузер выводились все наши скиллы.

// 2) - создать условие, проверяющее, старше ли юзер 18-ти лет. Если да - выводить
// одно сообщение, если нет - то совсем другое.

// 3) - создать функцию, которая будет искать корень переданного числа.

// ПРИМЕЧАНИЕ К 1-МУ ПУНКТУ: сообщений должно быть всего три, на каждый скилл!
// -----
// ПРИМЕЧАНИЕ К 2-ОМУ ПУНКТУ: сообщение, которое будет выводиться если юзеру
// больше 18-ти лет: "У тебя отличные шансы стать Frontend dev'ом!"; сообщение,
// которое будет выводиться если юзеру нет 18-ти лет: "Я удивлён, что ты так рано
// задумался о изучении Frontend'а...".
// -----
// ПРИМЕЧАНИЕ К 3-ЕМУ ПУНКТУ: надо сделать так, чтобы юзеру задавался вопрос:
// "Квадрат какого числа вы хотите найти?", и как он ответит это число сохраниться
// в спец. переменную, с которой мы будем проводить некоторые матем. операции.

// Ну что? погнали "допиливать" нашу функцию! создавать её буду без комментариев
// (в смысле, не буду прям всё-всё разжевывать - будут простенькие комментарии),
// так как времени у меня в обрез а урок затягивать еще на 1 день не охота.

function myFirstApp(name, age) {
	// Тот самый alert с интерполяцией
	alert(`Привет, меня зовут ${name} и это моя первая программа!`);

	function showSkills() {
		// Массив со скиллами
		let skills = ["HTML", "CSS", "JS"];
		// Цикл, "идущий" по всему массиву skills и выводящий alert для каждого скилла
	...
	function checkAge() {
		// Спрашиваем у пользователя его возраст и сохраняем в переменную
		age = prompt("Каков ваш возраст, мосье?");
		// Сами условия
		if (age > 18) {
			alert("У тебя отличные шансы стать Frontend dev'ом!")
		} else if (age < 18) {
			alert("Я удивлён, что ты так рано задумался о изучении Frontend'а..")
		}
	}

	function calcPow(num) {
		// Спрашиваем у пользователя, квадрат какого числа он хочет найти
		let powNum = prompt("Квадрат какого числа вы хотите найти, мосье?");
		// Выводим на экран спец. сообщение, в котором вычисляется квадрат числа
		alert(`Я нашёл квадрат вашего числа! вот он: ${powNum * powNum}`)
	}
...
// А на этом всё! урок также окончен. Файл стал большим, поэтому я создал новый.
// И да.. пару слов о сайте Konstruct - обновления перестали выходить из-за кое
// каких проблем с Фигмой, и только сегодня ночью удалось их решить! обновление
// сайта выйдет на след. уроке, так что - ждите! :)