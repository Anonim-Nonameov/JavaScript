...
// должны написать сразу третий, поставив точку с запятой. Третьим аргументом
// мы указываем то действие, которое будет выполняться каждую итерацию цикла,
// то есть каждый раз, когда цикл будет повторяться. Например, если мы хотим
// вывести цифры подряд, то нам с вами необходимо увеличивать каждую итерацию
// наш итератор на единичку. Сделаем это при помощи инкремента:
...
// То есть - сначала у нас итератор равен нулю, на следующей итерации он станет
// равным единице, а на следующей станет равным двойке, при следующей итерации
// он станет равным тройке и так далее. Но здесь можно указать более специфические
// вещи, например то, что итератор будет увеличиваться на два, а не на единицу:
...
// И заметьте, что поставив третий аргумент, мы не ставим точку с запятой - в
// этом нет никакой необходимости и это буде считаться ошибкой. У нас и без того
// уже есть три аргумента в цикле, это максимум! создание итератора, условие и
// действие при каждой итерации, это всё, что можно указать. Но нам необходимо
// увеличивать итератор лишь на единицу, так что инкремента вполне хватит :)
// Хорошо, теперь о том, что пишется в фигурных скобках. В фигурных скобках можно
// написать всё что угодно, любой код и любое действие! но если мы хотим просто
// пока что вывести в столбик числа от 0 до 9, то мы можем вписать в фигурные
// скобки команду console.log и в аргументы передать переменную i:
...
// Написав такой код, нам в консоль выдаст девять цифр - от нуля до девяти. На
// первой итерации цикла, цикл проверяет, меньше ли i чем 10 и если да, то цикл
// продолжает работу. Итератор (его ещё счетчиком называют) равен нулю и поэтому
// всё сработает, соответсвенно в консоль выводится ноль. После вывода в консоль
// числа 0, вновь проверяется переменная i, и если она меньше чем 10, то будет
// выполнен код внутри тела цикла. Итератор равен единице и поэтому всё также
// срабатывает, а в консоль выводится единица. Все эти действия повторяются до
// того момента, пока переменная i не станет равна девяти. Переменную с девяткой
// в виде значения проверят по условию и окажется, что цикл может продолжать свою
// работу, так как девять меньше чем десять. Но на следующей итерации значение
// переменной i станет равно 10 и проверится - окажется, что 10 не меньше, чем
// 10, они равны. Условие цикла принимает это как false и останавливает работу
// цикла, в итоге последним выведенным в консоль значением будет девятка.

// Ну вот, вместо того, чтобы создавать девять консоль.логов мы взяли и просто
// использовали цикл из трёх строк (а можно из одной строки, но об этом позже)!
// Это очень хорошо: задача выполнена, а кода мало. Но это слишком легкий пример,
// согласитесь - просто брать и выводить в консоль значения итератора.. не инте-
// -ресно и это как минимум. Нам надо больше практики, поэтому усложним задачу.
// Давайте вспомним наш старый код, где мы "мучали" пользователя вопросами и
// перепишем его здесь, но закомментировав, так как он будет.. мешать:
...
// В начале мы создаем массив answers, но не заполняем его, и вот, имеется пустой
// массив. После того как мы создали массив, мы заполняем его тремя вопросами. Но
// как мы его заполняем? а заполянем мы его следующим образом - сначала обращаемся
// к массиву и затем уже прописываем индекс, первым идёт индекс "ноль", так как мы
// заполняем массив с начала. После того как обратились к нулевому индексу, ставим
// первым значением массива ответ пользователя на вопрос "Как ваше имя?". Далее,
// мы берём и проделываем те же действия на новой строке, но вместо индекса "ноль"
// ставим индекс "один" и в команду prompt вписываем другой вопрос, а затем мы
// опять проделываем всё то же самое на новой строке, заменив индекс на следующий
// и поменяв вопрос. То есть мы заполнили массив ответами пользователя, но вопросы
// задаются на отдельной строке! но я напоминаю, что бывают такие случаи, когда
// таких вопросов может быть ОГРОМНОЕ количество, и заполнять массив таким образом
// (то есть ручным) нам не то, что надоест - это будет очень сложно! мы можем
// всё автоматизировать при помощи цикла. "Как лучше всего это сделать?" - можете
// вы меня спросить, а я вам отвечу, что конечно же у нас есть вопросы должны
// хранится где-то по порядку, и для этого подойдет массив. То есть мы просто
// возьмем и перепишем все наши вопросы в массив, делов то? :) Поэтому мы с вами
// должны взять и создать ЕЩЁ ОДИН массив, который будет называться "questions",
// что с англ. языка переводится как "вопросы". И тут же одно важное замечание -
// чтобы сразу создать несколько переменных, после одной уже созданной необходимо
// поставить запятую и прописать название другой, а после названия поставить опе-
// -ратор присваивания и само значение, при этом заново прописывать let (или же
// var, зависит от того, что вы используете) не придется. К чему я рассказываю
// об этом (пишу, точнее) - во первых, это полезная информация и её необходимо
// знать, а во вторых, нам необходимо создать две переменные - одна будет хранить
// массив answers, а вторая массив questions. Но массив questions мы заполним
// единожды и в одном месте, там же, внутри неё:
...
// Готово! теперь у нас есть два готовых массива, а первый будет заполняться со
// временем, сам - пользователь будет отвечать на вопросы и они будут идти в наш
// массив. Но как мы будем задавать эти вопросы? сейчас мы это и узнаем! мы
// будем использовать простой цикл для заполнения массива answers, но прежде
// всего мы должны его настроить, чтобы он брал нужный нам вопрос, подставлял
// его в команду prompt и записывал ответ пользователя в массив answers. Начнем
// с общей настройки цикла. В первую очередь мы создаём переменную-итератор,
// хорошо, а дальше нам надо придумать условие, при котором цикл будет продолжать
// свою работу. Нам в нашем случае необходимо продолжать работу цикла до тех пор,
// пока не закончатся вопросы. Для этого необходимо знать длину массива, то есть
// сколько элементов у нас внутри, и сказать циклу "работай, пока не закончатся
// эти вопросы!". Для того чтобы указать такое условие, после первого аргумента
// в цикле ставим точку с запятой и прописываем переменную i, таким образом мы
// обращаемся к ней, а после ставим знак "меньше". Нам надо указать массиву,
// чтобы он работал, пока i меньше чем количество элементов в массиве, для этого
// обращаемся к переменной, в которой хранится массив и ставим точку после самого
// названия переменной. После точки нам необходимо просто записать "length",
// это специальное свойство, которое говорит нам, сколько элементов содержится
// в той или иной сущности - массиве или же объекте. Выглядеть это будет так:
// questions.length - с таким условием, массив продолжит свою работу до тех пор,
// пока не закончатся все вопросы, или, если говорить простыми словами, пока
// i, наш итератор МЕНЬШЕ чем длина массива. Если он больше, то есть больше чем
// количество элементов в массиве (вопросов), то работа цикла остановится. И
// на этом еще не всё - остался последний аргумент, последняя гаечка - то, что
// мы будем делать каждую итерацию цикла. Ничего интересного делать мы не будем,
// просто увеличиваем переменную i на единицу, используя инкремент. Вот так всё
// будет выглядеть на данном этапе:

/*

for (let i = 0; i < questions.length; i++) {
	... (тут мы будем творить чудеса)
}

*/


// Отлично, идём дальше, к самому интересному - к тому, что будет творится в
// фигурных скобках, в теле цикла. Вспомните, как мы могли "достучаться" к
// каждому элементу в массиве? мы бы могли обратиться к массиву, поставить
// квадратные скобки и вставить цифру ноль:

// answers[0]

// Таким образом мы бы создали наш первый элемент в массиве, ответ, который
// поместился бы туда, если бы пользователь ответил на вопрос. Если бы нам
// надо было бы вставить в массив второй элемент, второй ответ, мы бы начали
// с новой строки и вписали бы в квадратные скобки цифру 1, если бы третий,
// то вписали бы цифру 2 и так далее. Вся суть в том, что мы УВЕЛИЧИВАЕМ индекс
// со временем, стараясь заполнить массив с нуля до n. Но мы же работаем с циклом,
// у которого есть итератор, который ТАКЖЕ увеличивается со временем! сначала он
// равен нулю, затем единице, затем двойке и так далее, почему бы единожды не
// записать вместо цифр в квадратных скобках переменную i? вот так:
...
// Теперь, когда наш цикл будет повторяться, вместо i будет подставляться соот-
// -ветствующая итерации цифра. На первом кругу это будет ноль, на втором это
// будет единица и так далее, и цифра, переданная в качестве индекса для массива
// answers - тоже, всё просто! хорошо, идём дальше. Что мы можем сделать дальше?
// дальше мы ставим оператор присваивания и после него прописываем команду prompt,
// таким образом у нас получается такая структура, что мы задаем вопрос юзеру
// через команду prompt и помещаем ответ в каждый отдельный элемент массива по
// порядку - сначала первый, затем второй и так далее. Но как мы будем задавать
// вопросы пользователю? для этого у нас есть массив questions, в который мы
// и записали все те вопросы, которые мы будем задавать пользователю через
// цикл. Поэтому внутри команды prompt мы должны что-то сделать с массивом
// questions, а именно - обратиться к нему через индексы и в качестве самого
// индекса прописать наш итератор, i:

// for (let i = 0; i < questions.length; i++) {
// 	answers[i] = prompt(questions[i]), '';
// }

// Почему мы поступили именно таким образом, почему вместо индекса в questions
// поставили i? здесь та же причина, что была и с answers - каждую итерацию
// наш итератор будет увеличиваться на единицу, и, соответственно, цифра, которая
// подставляется вместо i в квадратных скобках у переменных тоже будет меняться.
// Когда цикл будет повторяться, в i будет задаваться сначала 0, затем 1, затем
// 2 и так далее, и таким образом происходит соответствие: первый вопрос, который
// будет задан, это "Как ваше имя?". Пользователь отвечает на этот вопрос и,
// допустим, пишет "Артур", и теперь этот ответ, "Артур", отправляется в ПЕРВУЮ
// ячейку внутри массива answers. И всё! цикл полностью готов и может приступать
// к работе. Осталось только добавить команду document.write, чтобы выводить
// ответы пользователя на экран. Ответы хранятся в массиве answers, так что в
// круглых скобках обращаемся к этой массиву:

// document.write(answers);

// Готово. Теперь заходим в браузер, отвечаем на все вопросы и смотрим на саму
// страницу - на странице появляюся наши ответы. Таким образом мы не только взяли
// и сократили количество кода, так мы ещё и ОЧЕНЬ сильно облегчили работу! вы
// спросите, "почему?", а я вам отвечу, что теперь не важно, сколько у нас
// элементов в массиве - десять, двадцать, сотня, это уже не имеет значения.
// У нас прсото запускается цикл и всё, ничего лишнего! также массив можно
// выводить в консоль:

// console.log(answers);

// Теперь, когда пользователь ответил на все те вопросы которые мы ему задаём,
// его ответы, которые хранятся в массиве, выводятся не только на страницу,
// но и в консоль! но стоит также отметить, что такой подход очень часто исполь-
// -зуется в реальной работе: на веб-сайтах и веб-приложениях. Представьте себе
// секцию на сайте, простую обычную секцию с заголовком. В этой секции хранятся
// 6 элементов в двух рядах по 3 элемента на один ряд. При наведении на какой
// нибудь из элементов, в нём поднимается мини-меню с подробным описанием товара
// и кнопкой (ссылкой?), которая раскрывает ПОЛНОЕ описание, информацию о товаре.
// Вы бы увидев такую секцию с легкостью могли бы сказать, что все элементы в ней
// - практически одинаковые: та же ширина элементов, та же высота, шрифт, размер
// шрифта и так далее. Понятное дело, что информация ВНУТРИ элементов - разная,
// картинки товаров тоже разные, названия товаров тоже разные, так почему бы нам
// не поместить всё это дело в массив данных? можно создать один большой массив
// массив данных, который будет хранить в себе какие-то другие массивы. Первый
// подмассив будет содержать в себе, например, пути к картинкам. Следующий под-
//-массив будет содержать в себе описания товаров и так далее:
...
// Таким образом мы формируем структуру, в которой все данные хранятся по порядку,
// и в дальнейшем, при помощи цикла, мы сможем сформировать ту секцию, о которой
// я написал выше. План таков: эти данные будут	храниться на сервере, в том месте,
// куда наш пользователь доступа не имеет и которое мы с вами не видим. Оттуда же
// они и будут поступать к нам в тот момент, когда мы загружаем наш веб-сайт в виде
// массивов. Дальше, уже когда мы получили эти данные в виде массивов, мы при помощи
// цикла можем строить такие секции на сайте. Именно поэтому начинающему разработчику
// очень важно знать, как работают и как создаются циклы, так как это невероятно
// полезное умение разработчика, а еще циклы используются практически во всех 
// языках программирвоания, что подчёркивает их важность! идём дальше.

// Помимо всего этого вы также должны знать, что в JavaScript'е есть ещё два
// вида циклов, о которым мы сейчас и поговорим!

// Цикл с пост-условием
// A.S: перед тем как начать изучать этот цикл, я закомментирую предыдущий, где
// мы задаём вопросы пользователю и сохраняем ответы в массив данных, который
// выводится на страницу и в консоль, причина - мешает.

// Второй цикл в JavaScript называется "цикл с пост-условием". Что он значит?
// мы можем сказать себе, что что-то у нас сначала должно произойти, и только
// после этого должно провериться условие. То есть - этот цикл сначала выполняет
// действия, и только после того, как оно выполнилось, проверяет условие. Этот
// цикл записывается следующим образом: прописываем ключевое слово do, которое
// кстати переводится как "сделать, сделай" и т.д, и после него ставятся фигурные
// скобки, внутри которых и пишутся те действия, которые будет выполнять наш цикл.
// После фигурных скобок прописываем другое ключевое слово, while, и оно у нас
// переводится как "когда". После него мы ставим круглые скобки, внутри которых
// будет располагаться условие. Ещё стоит отметить, что здесь важно понимать,
// что условия в этих двух циклах формируются иначе, потому что нам понадобится
// отдельная переменная в качестве итератора, так как в цикле do-while НЕТ
// встроенного итератора. Вот как выглядит цикл на данном этапе:
...
// Прошу заметить - та переменная i, которая создается до самого цикла do-while,
// не имеет абсолютно НИКАКОГО отношения к той переменной i, которая находится
// в цикле for. Всё дело в том, эта переменная создается в глобальной области
// видимости, то есть использовать её теперь можно где угодно, а вот переменная
// i из цикла for создаётся только в области видимости цикла, и не доступна нигде
// кроме него. Теперь, когда итератор и цикл созданы, можно переходить к тому,
// что располагается в фигурных скобках. Для начала просто выполним логирование
// переменной i, то есть выведем переменную i в консоль. Далее, после того как
// эта переменная была выведена в консоль, мы берём и увеличиваем эту переменную
// на одну единицу каждую итерацию при помощи инкремента. А теперь переходим к
// условию. Условие будет таким - "пока i МЕНЬШЕ 10-ти":
...
// Всё, теперь наш цикл будет работать, и в консоль будут выведены все цифры
// от 0 до 9-ти. Это был цикл с пост-условием, то есть тот цикл, в котором
// сначала выполняются все действия и лишь потом проверяется условие, но есть
// и цикл с пред-условием, о нём мы и поговорим дальше.

// Цикл с пред-условием

// Итак, в этом цикле всё то же самое по синтаксису, как и в предыдущем цикле,
// но здесь мы сначала прописываем ключевое слово while, то есть мы сначала
// что-то проверяем, и еще кое что. Далее, после него мы ставим круглые скобки,
// в которых и будет хранится наше условие, но команда do в цикле с пред-условием
// не прописывается и после круглых скобок сразу идут фигурные. И еще важно
// отметить, что в этом ицкле ровно также, как и в предыдущем необходимо создавать
// переменную-итератор внешне, так как её нельзя создать внутри цикла:
...
// Прекрасно, а теперь приступим к написанию функционала. Первым делом идёт
// условие, и в качестве условия мы задаём следующее - переменная i меньше 10,
// то есть i < 10. Таким образом, наш цикл будет работать до тех пор, пока
// переменная i будет меньше 10. Теперь приступим к действиям внутри цикла,
// но на самом деле здесь будет всё то же самое, что и в предыдущем цикле: мы
// выводим в консоль переменную i и увеличиваем её каждую итерацию на единицу:
// P.S: перед тем как предоставить вам, читатель, код, я поставлю внизу команду
// console.log, для пробела в консоли браузера

// Пробел в консоли, не обращайте внимания!
console.log("Типа пробел.");
...
// В итоге, мы получаем в консоли числа от 0 до 9. Как здесь всё работает?
// ну, во первых мы меняем значение переменной i обратно на 0, потому что
// после работы предыдущего цикла, её значение стало равным десяти. Далее
// у нас идёт цикл. Цикл первым делом видит условие, читает его и если оно
// правдивое, то есть если оно возвращает true, то позволяет выполниться коду
// в фигурных скобках. В фигурных скобках мы выводим в консоль переменную i,
// наш итератор, и далее увеличиваем его на единицу после того, как вывели.
// Не первой итерации, переменная i равна нулю, она проверяется условием и
// если меньше десяти, то действия в цикле будут выполнены, и так как это
// действительно так, ноль меньше десяти, код внутри фиг. скобок срабатывает.
// И поскольку, повторю, переменная i равна нулю, следовательно в консоли мы
// получим цифру ноль, после этого она увеличивается на единицу. Теперь она
// равна единице и опять проверяется условием, и поскольку единица меньше
// десяти, код в фиг. скобках выполняется. По итогу в консоли получаем единицу,
// а переменная увеличивается. Всё это повторяется много раз, до тех пор, пока
// мы не получим в консоли девятку. После того как в консоль будет выведено
// число, цифра 9, переменная увеличивается на единицу и её значение теперь
// не 9, а 10. Выполняется проверка условия. 10 - НЕ меньше 10-ти, оно РАВНО
// 10-ти, поэтому цикл запрещает, чтобы действия внутри фигурных скобок были
// выполнены и на этом работа цикла прекращается.

// Эти циклы не такие уж и удобные, как цикл for, который мы можем очень
// гибко настраивать, тут даже используется внешняя переменная, и поэтому
// они используются намного реже. Но, это же не повод их не изучать! а
// теперь приступим к домашке...

// Но не здесь! я создал новый файл, поскольку этот стал слишком огромный :)