...
// С ним мы знакомы, но не с его "второй личностью". Он способен создавать новый массив,
// в который копирует либо часть, либо весь массив, не затрагивая исходный массив.
// Синтаксис тот же: arr.slice([start], [end]). end - не включая.

// Копируем часть массива

arrOne = ["Ваня", "Иштван", "Оля",];
// Начиная с 1-ой позиции "Иштван",
// до 2-ой позиции "Оля" (не включая)
...
// Скопировать весь массив, нет стартовой и конечной позиции.
arrFour = arrOne.slice();

console.log( );
console.log(arrFour);
// => [ "Ваня", "Иштван", "Оля" ] (node)

// Хоть arrOne и arrFour полностью схожи, но arrFour - его копия, это другой, отдельный
// массив, и никакого отношения к arrOne он не имеет.

// Метод concat.

// Создаёт новый массив, в который копирует данные из других массивов.
// Казалось бы, что тут такого удивительного то?
// Но он способен добавлять от себя НОВЫЕ значения!.. правда, в конец массива... 
// но всё же :-)
// Ладно, а как же синтаксис? а синтаксис прост: arr.concat(arg1, arg2, ...)

// P.S - название схоже со слово "Конкатенация".. интересно!

arrOne = ["Ваня", "Иштван", "Оля",];
arrTwo = arrOne.concat("Петя");

console.log( );
console.log(arrTwo);
// => [ "Ваня", "Иштван", "Оля", "Петя" ] (node)

// P.S - создавать новый массив "arrOne", а точнее перезаписывать его - было
// необязательно, т.к от прошлого метода slice() он не изменился.
// Но я всё же перезаписал его, чтобы не вызывало путаницы.

// Поиск в массиве

/*

Часто бывает так, что нам нужно что-то в массиве да найти.
В этом нам помогут следующие методы:

1) - indexOf (с ним мы уже знакомы, из урока про строки). Синтаксис: arr.indexOf(item, from).
Ищет item, начиная с индекса from, и возвращает индекс на котором был найден искомый элемент,
в противном случае вернет -1.

2) - lastIndexOf. Синтаксис: arr.lastIndexOf(item, from).
То же самое, что и indexOf - но ищет не слева направо, а справа налево.

3) - includes (тоже знакомы, с урока про строки вроде). Синтаксис: arr.includes(item, from).
Ищет item, начиная с индекса from, и возвращает true, если поиск успешен.
А что выдаст если поиск не увенчался успехом - нам уже известно :)

*/

arr = ["Ваня", "Иштван", "Оля",];

// indexOf (метод с которым мы уже знакомы)
...
// => -1 (т.к такого элемента в массиве нет, соответственно он его не нашел.)

console.log( );
console.log(arr.indexOf("Иштван", 2));
// => -1 (т.к второй индекс у элемента "Оля", а мы искали там "Иштван" - выдало -1.)

// includes (метод с которым мы уже знакомы)

console.log( );
console.log(arr.includes("Иштван"));
// => true (да, в массиве есть элемент "Иштван".)

console.log( );
console.log(arr.includes("Вася"));
// => false (нет, в массиве нету элемента "Вася".)

console.log( );
console.log(arr.includes("Иштван", 2));
// => false (нет, элемент с индексом "2" не "Иштван".)

// P.S - сюда не был добавлен метод lastIndexOf, за то что он работает так же как
// и его "старший брат".

// find & findIndex - поиск в массиве с определённым условием

/*

В случае если элементами массива являются объектами, нам на помощь придут методы
find и findIndex.
...
function - функция, которая выполняется для каждого элемента массива.
item - сам элемент массива, который сейчас проверяется
index - его ключ, позиция
array - наш массив

Лучше уж попрактиковаться, чем объяснять самому себе всё это в текстуальном виде.
Погнали!
*/
...
// => { name: "Коля", age: 18 } (node)

// Что тут произошло? нам вернуло объект, в котором содержится свойство age со
// значением 18. Это то нам и нужно! В условии мы написали "return item.age === 18",
// а это значит "верни тот item (элемент) в объекте, в котором содержится свойство
// age со значением 18, и ничего другого (===)!".
// Функция начала выплняться, проверять всё элементы массива, попутно смотря на условие,
// и увидела что в одном из элементов массива (в объекте) есть такое свойство age, и
// значение у него 18. Это true, поэтому перебор массива остановился и нам выдало объект!

// То же самое мы можем сделать и со стрелочными функциями, так намного короче:

resultOne = arr.find(item => item.age === 18);

console.log( );
console.log(resultOne);
// => { name: "Коля", age: 18 } (node)

// Если так посмотреть, то в принципе особой разницы то и нет!)

// findIndex

// У метода findIndex тот же синтаксис что и у find, но они отличаются.
// Из названия всё ясно - если find вовзращал нам или item, или index, или array -
// то findIndex вернет именно индекс того объекта, на котором произошло совпадение.
// Вспомним код выше, с find - он нашел объект { name: "Коля", age: 18 } и вернул
// нам его, но мы не знаем какой у этого объекта индекс, а ведь таких объектов
// может быть хоть 100!
// findIndex же вернет нам индекс, и поможет найти искомый объект.
...
// Всё правильно, у того объекта 1-ый индекс! Метод отработал без ошибок.

// Метод filter

/*

Метод filter очень похож на find(Index), но он полностью отличается.
...
Как мы видим - синтаксис тот же, но делает он - совсем другое.
А теперь практика - погнали!
*/

let result = arr.filter(function(item, index, array) {
    // Я хочу получить все объекты, у которых свойство age больше или равно 18
    return item.age >= 18;
});

console.log( );
console.log(result);
// => [ { name: 'Вася', age: 36 }, { name: 'Коля', age: 18 } ] (node)

// Нам вернуло сразу два объекта - первый совпадает по условию БОЛЬШЕ или равно
// (age = 36), а второй по условию больше или РАВНО (age = 18).

// Сортировка массивов

// Метод sort(func)

// Часто у верстальщиков бывают случаи, когда им необходимо "отсортировать" массив,
// и чтобы отсортировать массив им помогает метод sort().
// Он просто сортирует массив на месте, меняя в нём порядок элементов.

// Сортировка слов

arrOne = ["Иштван", "Ваня", "Оля"];

console.log( );
console.log(arrOne.sort());
// => [ "Ваня", "Иштван", "Оля" ] (node)

// Как мы видим, sort отсортировал слова в алфавитном порядке.

// Сортировка чисел

// При сортировке чисел, может произойти недоразумение.
...
// Мы получили 1, 22 и 8, а ожидали "1, 8, 22"!
// Всё дело в том, что элементы сортируются как строки.
// Для строк применяется лексикограчифеский порядок, и действительно выходит так,
// что "8" больше чем "22".
...
// И что же нам делать?
// А в этом нам поможет собственная функция сортировки!

// Для начала пересоздадим наш массив, чтоб не запутаться:

arrTwo = [8, 22, 1];

// Теперь выведем наш массив с методом sort в консоль:
...
// Не думаю что здесь что здесь надо объяснять, как работает функция.
// Всё итак здесь предельно ясно.
// Далее, мы передаем в метод sort нашу функцию:
...
// Это всё конечно же круто, но во первых: много кода. Во вторых: в результате
// у нас два раза выводится надпись "Сравниваем 8 и 22".
// Исправить это можно, и как - сейчас увидим!
// Более того, я еще покажу как уменьшить код функции:
...
// Если a меньше b - выдаст -1, если они станут равными - выдаст 0, а если
// a больше b и в результате вычитания всё равно a будет больше - выдаст 1.

console.log( );
console.log(arrTwo.sort(compareNumeric2));
// => {
/*
        "Сравниваем 8 и 1"
        "Сравниваем 22 и 8"
...
// А можно ЕЩЕ БОЛЬШЕ уменьшить код функции, используя функцию-стрелку:
...
// Ну вот - и в кносоль результат вывели, и проблему с сортировкой решили!

// Метод reverse

// Я не знаю почему, но этот метод включен в раздел "сортировка массивов".. но не об этом.
// Mетод reverse сортирует массив наоборот - берет и меняет порядок элементов
// на обратный, в реверс.

arrOne = ["Ваня", "Иштван", "Оля"];

console.log( );
console.log(arrOne.reverse());
// => [ "Оля", "Иштван", "Ваня" ] (node)

// Преобразование массивов

// Метод map

// Метод map вызывает функцию для каждого элемента массива (с такими методами
// мы уже достаточно хорошо знакомы, например find и findIndex) и возвращает НОВЫЙ
// массив, а именно - массив с результатами выполнения этой функции.

// Синтаксис тот же, что и в случае с find и findIndex:
...
result = arr.map(function(item, index, array) {
    // Хотим получать первую букву с каждого текстового элемента массива
    return item[0]
});
...
// И мы также можем сократить наш код, юзая стрелочную функцию:

result = arr.map(item => item[0]);
...
// Метод split преобразовывает строку в массив (да, в JS даже такое возможно),
// и разделяет ее по заданному разделителю.

// Синтаксис: arr.split([разделитель])

let str = "Ваня,Иштван,Оля";
arr = str.split(",") // запятая - разделитель

console.log( );
console.log(arr);
// => [ "Ваня", "Иштван", "Коля" ] (node)

// А еще можно ограничить кол-во объектов которые попадут в наш массив:
...
// Так.. файл стал слишком огромным, поэтому создаю другой.