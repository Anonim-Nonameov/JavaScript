...
// А остальное допишу позже, а сейчас - !!!ВАЖНОЯ ИНФАРМАЦЫЯ!!!:
// В последнее время, я всё больше начал путаться в уроках - всё дело в том, что у меня бывает несколько
// занятий по JS (codeBasics, Udemy и этот же курс), а также HTML (HTML Academy, курс от Жеки),
// и я просто иногда забываю делать какие-то из них (или у меня иногда просто не хватает времени),
// из-за этого у меня в гитхабе получается какая-то каша. А уже к вечеру, будучи уставшим я просто забываю
// пушить в гитхаб свои занятия! Например, из-за отступа между курсом на HTML Academy и курсами Жеки, я
// иногда по 2-3 дня занимался именно там, а про Жекин курс по HTML и вовсе забыл, в итоге - какой-то
// урок до сих пор не пройден, или не повторен конспект и т.д.
// И я решил - один день буду проходить курс по JS от Жеки, второй день курс по JS на Udemy и HTMl от
// HTML Академии, а codeBasics я буду повторять исключительно на выходных.