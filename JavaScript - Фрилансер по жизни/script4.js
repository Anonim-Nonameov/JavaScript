...
// Во первых, 1. - мы уже знаем, что основой любого HTML документа являются теги,
// и в соответствии с DOM каждый тег явялется объектом. Вложенные теги являются дочерними и потомками
// своего родителя, и даже текст комментариями является объектом, и комментарии в этом плане
// тоже никак не хотят отставать - они тоже объекты! бсолютно все эти объекты доступны нам из
// JavaScript и мы можем ими управлять.

// 1.1 (DevTools и дерево документа) - Чтобы лучше визуализировать себе дерево документа мы можем открыть
...
// 1.2 (Взаимодействие с DOM и навигация по нему) - Отлично, что такое дерево документа мы поняли, но как
// же с ним взаимодействовать? Все операции с DOM начинаются с объекта document, это основная точка входа
// в DOM, изнего мы можем получить доступ К ЛЮБОМУ узлу. Для начала разберем навигацию по DOM-элементам.
// Как сказано выше, объект document является основной точкой входа в DOM, и самые верхние (высшие) элементы
// доступны нам как его свойства:
