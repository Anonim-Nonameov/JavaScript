...
html {
	...
	body {
		...
		h1 {
			...
			Text: "Браузерное окружение."
			
			Text: "Работа с" span.yellow {
				Text: "DOM."
			}
		}
...
// его заиспользуем нам выдаст объект head. А если же мы хотим обратиться к следующему
// элементу (также по отношению к body), то мы должны использовать nextElementSibling,
// и когда мы его заиспользуем нам выдаст null, так как после body ничего нет (как
// оказалось, это основная причина выдачи null). А если мы хотим получить родителя,
// то мы должны использовать свойство parentElement, и когда мы его заиспользуем
// нам выдаст объект html, так как объект body находится внутри него.

// А остальное я допишу через... 3 дня! :)