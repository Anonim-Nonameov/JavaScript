...
// изучении этих методов, так как они просто не замечают этой буквы, или же просто
// пропускают её, приговаривая что "это ошибка". Именно по этой причине, те методы,
// которые мы будем изучать дальше, надо рассматривать очень внимательно! еще раз
// повторю: не пропускайте никаких букв в названии методов - приведет к ошибке!
// А мы идем дальше. getElementsByTagName ищет по имени тега и возвращает ЖИВУЮ
// коллекцию (живая коллекция это та, которая отражает текущее состояние DOM!).
// Искать он может как в объекте document, так и в любом другом объекте. Здесь
// всё точно также, как и с предыдущими методами - никаких различий НЕТ :)
// ВнИмАнИе!!! (на этом моменте типа барабанная дробь) - пример использования:
...
// страницы, и да - потому что эта коллекция как-бы живая, элементы в ней будут
// меняться в зависимости от того, менялись они или нет. Т.е - если мы поменяем
// какой-то элемент li, он изменится не только на странице - он изменится и
// и в нашей коллекции тоже.
