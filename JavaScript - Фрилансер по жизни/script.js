// ВАЖНО!!! -
// Теперь я буду оставлять в коде такие записи: console.log( ).
// Это для того, чтобы в терминале были хоть какие-то промежутки между выведенными
// значениями, иначе можно запутаться.
// Добавлено: 25.06.2021

// Домашка

let userName = 'Вася';

let user = userName;

console.log(user);
// Объявляем перем. userName (a) со знач. Вася (1), затем объявляем перем. user
// (b) со значением переменной userName (a) (a = 1, b = a), затем выводим в
// консоль значение переменной user (1). 

// const myAge  = 36;
// myAge = 18;

// Этот код выведет ошибку, ведь константу изменить нельзя!


// Типы данных

let string = 'string'; // Строка - string
// Ну, строкой может быть что угодно что находится в кавычках - "" ''. Даже
// пустые кавычки - строка.

let undef; 
console.log(undef);
// Неопределённый - undefined. Выдаётся, когда мы хотим вызвать переменную
// которая на самом деле есть, но значение - неопределено.

let var_null = null; // Ничего - null. 
// Специальное значение, которое - ничего. Переменная объявлена, но значение -
// ничего, ноль.

let yes = true, no = false; // Логический тип, буллин - boolean.
// Имеет лишь 2 значения - true (правда) и false (ложь).

let trueOrFalse = 58 < 18; 
console.log(trueOrFalse);
// Переменная может сама получать значение false или true. Тут она сама получила
// значение false, ведь 18 МЕНЬШЕ чем 58.

let number = 200; // Число - number
// Число может содержать в себе любые цифры, числа, а также числа с плавающей
// точкой (22.5).

let infin = 15/0; // infinity - number.
// Математическая бесконечность. Есть также -infinity, минус-бесконечность.
// Получить её можно написав let infin = -15/0 и вывести в консоль.

let notAnNumber = 'Hello' / 5; console.log(notAnNumber);
// NaN (not an number - не число) - number. Выдаёт такую вычислительную ошибку
// обычно когда пытаются разделить строку на 5 и т.д. Тип данных - число, хоть
// NaN значит не число. Парадокс.

let bigInteger = 12345678901234567890123456789n; 
console.log(bigInteger);

// bigint - число, но огромное. Выдаётся bigint потому, что в JS тип данных
// number не может содержать числа больше чем 9007199254740991 или меньше
// чем -9007199254740991.

let userInfo = {
 name: 'Ismail',
 age: 14
}
console.log(userInfo);
// Объект - object. Объект это что-то вроде переменной, которая хранит в себе
// несколько значений, свойств. Свойства состоят из бар, двоеточие и значение.
// Тут name - ключ, 'Ismail' - значение.

let id = Symbol('id');
// Символ - symbol. Уникальный айди объекта, скажем так.. его собт. имя.

let funcVar = function name(params) {
  // некий код  
}
// Функция - function. Тот же объект, но просто разный тип данных.


// Преобразование типов (замена их данных)

let userAge = 58; // => число console.log(userAge); 

userAge = String(userAge) // => теперь тип данных переменной - строка.
console.log(userAge);

// Операторы

// В JS есть три группы операторов:
// Базовые операторы
// Операторы сравнения
// Логические операторы

// Термины «унарный», «бинарный», «операнд».

// Операнд - то к чему применяется оператор, операнды ещё
// называют аргументами.
// Например:

userAge = 39 - 9; // => Числа 39 и 9 - операнды

let h = 200, y = 10;

let userHeight = h - y; // => Переменные h и y - операнды

// Бинарными называются операторы которые применяются
// к двум операндам.
// В примере выше оператор вычитания ( - ): бинарный оператор.

// Унарный опператор - который применяется лишь к 1 операнду.
// Например:
let userCash = -800;

// Математические операторы

// В JS есть мат. операторы.
// Например:

let x;

// Оператор сложения: +
x = 5 + 8;

// Оператор вычитания: -
x = 13 - 8;

// Оператор умножения: *
x = 5 * 2;

// Оператор деления: /
x = 10 / 2

// Оператор взятия остатка от деления: %
x = 11 % 3;
// Проверка => 11 / 3 = 3 (2 ост.)

// Оператор возведения в степень: **
x = 3 ** 2;

// Спец. возможности операторов

let resultOne;
let resultTwo;
let resultThree;
let resultFour;
let resultFive;

// Применение оператора сложения к строкам

resultOne = "Ученик" + " " + "по" + " " + "жизни" 
console.log(resultOne);
// Вывод => Ученик по жизни

// Если в выражении есть хоть одна строка (т.е -
// хотя бы один операнд будет строкой) - то конечный
// результат тоже будет строкой.

// ПРИМЕЧАНИЕ: порядок значения не имеет,
// т.е - не важно, первым операндом будет 
// число или строка.

resultTwo = "Ученик " + 18;
console.log(resultTwo);
console.log(typeof resultTwo);
// Вывод => Ученик 18 - тип String.

// Меняем строку и число местами:
resultThree = 18 + " Ученик"
console.log(resultThree);
console.log(typeof resultThree);
// Вывод => 18 Ученик - тип String.

// Казусы.

// Иногда бывает так, что мы из 1-ой цифры можем 
// получить сразу несколько десятков или даже сотен.
// Например - из 2 получить 22, или 222:

resultFour = 2 + "2";
console.log(resultFour);
// Вывод => 22

// Операции до сложения со строкой.

// Операции до сложения со строкой выполняются
// как обычно, даже если мы перед строкой поставим
// какие-то числа для сложения:

resultFive  = 18 - 8 + " Ученик";
console.log(resultFive);
// Вывод => 10 Ученик

// Сначала будет вычитано 18 - 8 и получено 10,
// а затем эта десятка будет сложена со строкой.

// Работа строк и чисел с операторами

resultOne = "25" - 5;
console.log(resultOne);
console.log(typeof resultOne);
// Вывод => 20 - тип Number

resultTwo = 10 * "80";
console.log(resultTwo);
console.log(typeof resultTwo);
// Вывод => 800 - тип Number

// Недопустимые операции:

resultThree = 3 * "Ученик";
console.log(resultThree);
// Вывод => NaN

resultThree = "Ученик" * 3;
console.log(resultThree);
// Вывод => NaN

// Унарный оператор сложения: +
// Здесь оператор сложения играет
// роль преобразователя строк в числа

// Со строками

resultOne = +'25';
console.log(resultOne); 
console.log(typeof resultOne);
// Вывод => 25 - тип Number

// С числами

resultTwo = +10;
console.log(resultTwo);
console.log(typeof resultTwo);
// Вывод => 10 - тип Number (ничего не меняется)

// Ещё пример:

let users = "25";
let admins = "10";

console.log(users + admins);
// Вывод => 2510

console.log(+users + +admins);
// Вывод => 35

// Почему нам вывелось 35? потому что мы применили
// унарный оператор сложения к переменным и сделали
// их тип числом, а числа - складываются.

// Простыми словами: 25 + 10 = 35

// Приоритет операторов

// Разный приоритет:

resultOne = 2 - 1 * 5;
console.log(resultOne);
// Вывод => 3 (1 * 5 - 2)
// Тут выполнится сначала умножение а затем вычитание.

// Приоритет скобок:

resultTwo = (1 * 5) - 2;
console.log(resultTwo);
// Вывод => 3

// Равный приоритет:

resultThree = 5 - 2 - 1;
console.log(resultThree);
// Вывод => 2

// Оператор присваивания

let a = 1 + 2;
// Значение переменной a равно 1 + 2 (3).
let b = 2;
// Значение переменной b равно 2.
let result = 8 - (a = b + 3)

console.log("Результат в скобках: " + a);
// 5
console.log("Общий результат " + result);
// 3

// Присваивание по цепочке

resultOne = resultTwo = resultThree = 1 + 2;

console.log(resultOne);
console.log(resultTwo);
console.log(resultThree);

// Проще -

resultOne = 1 + 2;
resultTwo = resultOne;
resultThree = resultOne;

console.log(resultOne);
console.log(resultTwo);
console.log(resultThree);

// Сокращённая запись вычислений с присваиванием

users = 5;
users = users + 3;
users = users * 2;

// Можно записать так:

users = 5;
console.log(users);
// Вывод => 5

users += 1 + 2;
console.log(users);
// Вывод => 8

// Инкременты & декременты

// Инкремент "++" - увеличивает значение переменной на 1:

let addUser = 2;
addUser++;
console.log(addUser);
// Вывод => 3

// Работает так же как и:
// addUser = addUser + 1;

//Или как:
// addUser +=1;

// Декремент "--" - уменьшает значение переменной на 1:

let removeUser = 2;
removeUser--;
console.log(removeUser);
// Вывод => 1

// Примечания - инкремент & декремент нужно прописывать
// на кончике переменной, и инк./дек. можно применить только
// к переменной. Попытка использовать его на значении вроде
// 5++ приведёт к ошибке синтаксиса.

// Инкременты и декременты можно прописывать как до так и
// после переменной:

// Постфиксная форма -
let usersCounter = 0;
let newUsers = usersCounter++;
console.log(newUsers);
// Вывод => 0
// Вывелось 0 потому, что переменная newUsers получает
// первое значение usersCounter ещё ДО работы оператора.

// Префиксная форма -
usersCounter = 0;
newUsers = ++usersCounter;
console.log(newUsers);
// Вывод => 1
// А вот тут уже вывод 1, потому что сначала сработал
// оператор а затем уже было присвоено значение.

// Итог:
// Если результат оператора не используется, а нужно
// лишь увеличить или уменьшить значение переменной,
// тогда без разницы, какую форму использовать:

usersCounter = 0;
usersCounter++;
console.log(usersCounter);

// Но если нужно увеличить/уменьшить и при этом
// получить значение переменной до увеличения - 
// мы используем постфиксную форму:

usersCounter = 0;
console.log(usersCounter++); // => 0
console.log(usersCounter); // => 1

// Инкремент и декремент можно использовать в любых
// выражениях, при этом их приоритет будет выше
// чем у большинства других арифметических операций:

usersCounter = 0;
newUsers = 2 * ++usersCounter;
console.log(newUsers);
// Вывод => 2 (2 * 1)

// В этом примере сначала выполнится инкремент,
// т.е - переменная usersCounter станет равна 1,
// и лишь после этого будет выполнено умножение.

// Более читабельная запись:

usersCounter = 0;
++usersCounter; // теперь значение usersCounter равно 1
newUsers = 2 * usersCounter;

console.log(newUsers);
// Вывод => 2 (2 * 1)

// Оператор запятая ,

// Оператор запятой позволяет нам вычислять сразу
// несколько выражений, разделяя их запятой.
// Каждое выражение выполняется, но будет
// возвращён лишь результат последнего выражения.

usersCounter = (8 + 2, 19 + 1);
console.log(usersCounter);
// Вывод => 20
// Был выдан результат последнего выражения (19 + 1).

// Операторы сравнения:

/*

Больше: a > b
Меньше: a < b
Больше или равно: a >= b
Меньше или равно: a <= b
Равно: a == b
Не равно: a != b
Строгое равно: a === b
Строгое не равно: a !== b

*/

// Операторы сравнения возвращают логический
// тип данных со значением true (истина) & false (ложь):

console.log(2 > 1);
// Вывод => true

console.log(4 < 2);
// Вывод => false

console.log(58 == 36);
// Вывод => false

console.log(8 != 9)
// Вывод => true

// Присваиваем результат переменной:

result = 11 > 10;
console.log(result);
// Вывод => true

// Сравнение строк

// JS сравнивает строки по алфавитному порядку,
// количеству символов, или регистру.

// Алфавитный порядок:
console.log('Б' > 'А');
// Вывод => true (Б идёт в алфавите после А, она больше)

// Алфавитный порядок:
console.log('Скрипт' > 'Скрипка');
// Вывод => true
// Первые 5 символов (скрип) одинаковы, и только 
// буква Т в слове скрипт - дальше букв к & a.

// Кол-во символов:

// ПРИМЕЧАНИЕ ПЕРЕД ПРИМЕРОМ!
// Если в строках одинаковый алфав. порядок и ни 1-ая
// и ни 2-ая строка друг друга в алфавите не опережают,
// то JS считает кол-во символов.
// И та строка, в которой будет больше символов - истинна (true).

console.log('Слайдер' > 'Слайд')
// Вывод => true
// Если количество смиволов у строк разное, то будет
// больше та строка, у которой кол-во символов больше при всех
// остальных равных.

// Регистр:
// ПРИМЕЧАНИЕ ПЕРЕД ПРИМЕРОМ!
// Регистр в JS имеет значение, причем в JS (а точнее в Юникоде,
// который использует этот язык) маленькие буквы БОЛЬШЕ чем большие.

console.log('Ученик' > 'ученик');
// Вывод => false, ведь слова с маленькой буквы - больше
// чем с большой (по крайней мере в Юникоде).

// Алгоритм сравнения строк прост:

/*

1) Сначала сравниваются первые символы строк.

2) Если 1-ый символ первой строки больше(меньше), чем первый символ
второй, то первая строка больше(меньше) второй. Сравнение заверш.

3) Если первые символы равны, то таким же образом сравниваются уже 
вторые символы строк.

4) Сравнение продолжается, пока не закончится одна из строк.

5) Если обе строки заканчиваются одновременно, то они равны.
Иначе, большей считается более длинная строка.

*/

// Сравнение разных типов

// Преобразование в число
console.log('58' > 10);
// Вывод => true

console.log('007' == 7);
// Вывод => true

// Так происходит из-за того, что JS при сравнении разных типов
// преобразовывает строки в числа.

// В 1-ом примере строка '58' превратилась в число 58, а как мы
// знаем - число 58 больше чем 10.
// Во 2-ом примере строка '007' превратилась в число 7, а 7 и 7 - равны.

// Логическое значение true становится 1, а false - 0.
console.log(true == 1);
// Вывод => true

console.log(false == 0);
// Вывод => true

// Интересные ситуации

let itemA = 0, itemB = '0';

console.log(Boolean(itemA));
// Вывод => false
// Почему так произошло? потому что 0 - это false.

console.log(Boolean(itemB));
// Вывод => true
// Почему true? потому что 0 - это уже строка, а строка
// у нас в данный момент не пуста, в ней что-то есть.

console.log(itemA == itemB);
// Вывод => true
// Выводится true потому, что JS преобразовывает (как я уже
// и писал об этом) строки в числа, и вышло так что мы говорим:
// "Ноль равно ноль" или "Ноль равен нулю", а это правда, они равны.

// Строгое сравнение

// Оператор строгого равенства поможет избежать подобных ситуаций (
// как в примерах выше) потому, что оператор строгого равенства или не равенства
// проверяет равенство без приведения, преобразования типов.

console.log(0 === false);
// Вывод => false
// Вывелось false потому, что оператор с.р не приводит типы.
// В нашем случае - 0 это тип number, а false тип boolean, и поэтому
// вывелось false - ведь number и boolean два разных типа.

console.log('007' === 7);
// Вывод => false
// Тут то же самое: строка и число два разных типа.

console.log('58' !== 58);
// Вывод => true
// А вот тут уже true, потому что строка (string) 58 не
// равна числу (number) 58.

// Сравнение с null и undefined

console.log(null === undefined);
// Вывод => false
// Вывелось false, потому что null - тип данных object, а undefined -
// тип данных undefined.

console.log(null == undefined);
// Вывод => true
// Почему вывелось true? потому, что они оба возвращают "нет значения",
// они равны друг другу и не равны никаким другим значениям, это спец. правило языка.

// Т.е - они чем-то похожи на false, только вот null может быть назначен
// программистом, а undefined - назначается самим интерпретатором, если переменная
// не объявлена или мы пытаемся вернуть свойство какого-то объекта..
// но это уже совсем другая история.

// Сравнение null и undefined

// При использовании математических операторов и других операторов сравнения < > <= >=
// Значения null & undefined преобразуются к числам: null становится 0, а undefined - NaN

// Сравнение null и 0

// При использовании арифметических операторов или других операторов, таких
// как равно, больше или равно и т.д - идёт преобразование к числам, в итоге
// null становится нулём а undefined становится NaN

console.log(null > 0);
// Вывод => false, null не может быть больше нуля,

console.log(null == 0);
// Вывод => false, null не может быть равен нулю,

console.log(null >= 0);
// Вывод => true, null будет больше либо равен нулю.
// Результат этого выражения говорит о том, что "null большие или равен нулю".

// Причина в том, что нестрогое равенство и сравнения > < >= <= работают по разному.
// Сравнения преобразуют null в число, видя его как нолик (0).
// Поэтому последнее выражение null >= 0 истинно, а null > 0 ложно.

// С другой стороны, для нестрогого равенства == значений undefined и null есть
// особое правило: эти значения ни к чему не приводятся, они равны друг другу и не равны
// абсолютно ничему другому. Поэтому второе выражение, null == 0 - ложно.

// Сравнение undefined и 0

// При сравнении undefined c 0 всё будет false, ложью.

console.log(undefined > 0);
// Вывод => false

console.log(undefined < 0);
// Вывод => false

console.log(undefined == 0);
// Вывод => false

// Первое и второе сравнение вернут false потому, что undefined преобразуется в NaN, а NaN -
// специальное числовое значение, которое возвращает false при любых сравнениях.

// Третье сравнение возвращает false потому, что undefined равно только null и ничему больше.

/* 
Чтобы избежать проблем при сравнении нужно быть очень осторожным к любому
сравнению с null и undefined, кроме случаев со строгим равенством ===.

Не использовать операторы сравнения:
больше, > - больше или равно, >= - меньше, < - меньше или равно <=;
с переменными, которые могут принимать значения null и undefined, разве
что ты точно знаешь что делаешь.
Ну, в любом случае, если переменная может принимать эти значения,
то следует добавить для них отдельные проверки.
*/

// Логические операторы

// В JS есть ещё одна группа операторов, логические.
// Несмотря на их название, они применяются к ЛЮБЫМ типам данных -
// будь то строки, числа, бигинтегер или буллин.

// Оператор ИЛИ, записывается как две вертикальные черты.
// Процесс его работы следующий:

/*
1) Вычисляет операнды слева направо.

2) Каждый операнд конвертирует в логическое значение.
Если результат true, останавливает и возвращает исходное значение этого операнда.

3) Если все операнды являются ложными(false), возвращает значение последнего из них.
*/

console.log(true || true);
// Вывод => true

console.log(false || true);
// Вывод => true

console.log(true || false);
// Вывод => true

console.log(false || false);
// Вывод => false

console.log(30 > 70 || 10 > 40);
// Вывод => false

console.log(1 * 3 > 10 || 6 * 6 > 200);
// Вывод => false

// Простыми словами - этот оператор будет проверять всё до последнего,
// и если найдет true - сразу выдаст true, даже если будет хоть 10 false'ов и лишь 1 true.
// НО! если не будет ни одного true - выдаст false, т.к просто не нашел true.

// Разные типы

console.log(1 || 0);
// Первые true это 1, выводит в консоль 1

console.log(true || 'ученик');
// Первые true это true, выводит в консоль true

console.log(null || 58);
// Первый true это 58, выводит в консоль 58

console.log(null || 'ученик' || 0);
// Первый true это 'ученик', выводит в консоль строку 'ученик'

console.log(undefined || '' || null || 0);
// true вообще нету, выведет в консоль 0

// Присвоение значения в переменную

userName = '';
let userNickName = "Ученик";

user = userName || userNickName || "Без имени";
console.log(user);

// Переменная userName пуста, а переменная userNickName равна строке "Ученик".
// Перменная user равна или userName, или userNickName, или строке "Без имени".

// Как понять, чему она (user) равна? легко - userName выдаст false, строка то пустая,
// userNickName равна строке, строка не пуста и равна true - выведет строку в
// переменной userNickName.

// Сокращённое вычисление, условие.

admins = 0;
users = 5;

admins > users || users++;
console.log(users);
// Вывод => 6

// Почему 6? потому, что мы применили оператор сравнения или и говорим:
// "admins (0) больше users (5), или users (5) инкремент".

// Инкремент сразу же увеличил значение переменной на 1, получили 6.
// Так как первое выражение ложно, ведь 0 не больше 5, выводит 6.
// Мы сделали своеобразное условие, при котором будет выводится некое значение.

// А можно изменить значение admins на 10, тогда вот что будет:

admins = 10;
users = 5;

admins > users || users++;
console.log(users);

// Первое выражение истинно, ведь 10 больше 5. И как оно вычислится - на этом всё,
// оператор или не перейдет к переменной users и не увеличит её, она останется 5.

// Оператор && (И)

/*
Следующий логический оператор это И. Пишется как два амперсанда (&&) и выполянет
следующие действия:

1) Вычисляет операнды слева направо.

2) Каждый операнд преобразует в логическое значение.
Если результат false, останавливается и возвращает исходное значение этого операнда.

3) Если все операнды были истинными, возвращает значение последнего.

*/

// Булевые значения

console.log(true && true);
// Вывод => true

console.log(false && true);
// Вывод => false

console.log(true && false);
// Вывод => false

console.log(false && false);
// Вывод => false

// Простыми словами - этот оператор работает как оператор или,
// только наоборот - если или выводил true если находил, то этот оператор
// выводит false если находит.

// Разные типы

console.log('Ученик' && 0 && 2 & 3);
// Вывод => 0, ведь 0 это false

console.log(1 && 2 && null && 3);
// Вывод => null, он тут единственный вернёт false.

console.log('15' && '42');
// Тут все значения true, поэтому вернёт значение последнего - 42.

// Приоритет оператора && больше чем оператора ||

console.log(1 && 0 || 2 && 1);
// Вывод => 1

// Почему вывелось 1? потому, что сначала сработал оператор И.

// Он посмотрел на первое выражение - 1 и 0. 0 это false, значит 0 false.
// А потом посмотрел на второе выражение - 2 и 1, оба true - значит вернёт
// последнее значение - 1.
// Т.е - false || true.

// Подобие условия

users = 1;

(users > 0) && console.log(`Пользователей ${users}`);
// Вывод => Пользователей 1

/*

Весь код можно объяснить вот так:

"У нас есть переменная users которая равна 1.
И дальше есть выражение: users (1) больше 0, а это true.
Поэтому, мы переходим ко второму операнду -
console.log(`Пользователей ${users}`), его то и выводим."

Можно представить, что вместо слова users в вызове console.log() стоит 1.
Из этого становится ясно, что мы выведем "Пользователей 1".

*/

// Оператор ! (НЕ)

/*

Выполняет следующие действия:

1) Сначала приводит аргумент к логическому типу (true/false).

2) Затем возвращает противоположное значение.

*/

// Булевые значения

console.log(!true);
// Вывод => false, ведь false - противоположное значение true.

// Разные типы

console.log(!null);
// Вывод => true
// На самом деле null вернул бы false, но т.к тут есть оператор не - он вернул true.

console.log(!1);
// Вывод => false, ведь мы превращаем 1 в логическое значение (а 1 это true),
// а противоположность true - это false.

console.log(!'');
// Вывод => true
// На самом деле пустая строка вернула бы false, но её противоположность: true.

console.log(!'ученик');
// Вывод => false, ведь строка то не пустая - поэтому она true,
// а противоположность true: false.

// Оператор не ( ! ) позволяет преобразовывать типы в Boolean:

console.log(!!'ученик');
// Вывод => true. Де-факто строка не пуста, она и есть true, но один оператор
// НЕ сделал бы её false, а !! - нет.

// Ну или вот так:

console.log(Boolean('Ученик'));
// Вывод => true

// Оператор сравнения с null и undefined ( ?? )

// Оператор ?? возвращает первый аргумент, если он не null / undefined,
// в противном случае (если 1-ый аргумент null/undefined) - возвращает второй.

let name;
console.log(name ?? "Без имени");
// Вывод => Без имени

// Мы объявили переменную но не дали ей значение, в данный момент она undefined.
// Оператор ?? увидел это, и выдал нам второй аргумент.

// Примечание - если оба операнда null или undefined, то оператор ??
// выводит значение последнего.

// Домашка

// 1)
console.log('35' + - "22") // F, 32-22

// 2)
console.log('35' * '22'); // T, 770

// 3)
// Выводит ошибку.

// 4)
usersCounter = 0;
newUsers = usersCounter++;
console.log(newUsers); // F, 0

// 5)
console.log(!false && 11 || 18 && !''); // F, 11

// 6)

name = 0;
console.log(name ?? "Без имени") // 0

// if (если) и else (еще)

// Инструкция if и else это некий блок кода, который будет выполнятся только
// при определенном значении true.

// Синтаксис

let message = "Привет, ученик!"

if (2 > 1) {
  // Код выполняется только если
  // выражение в скобках вернет true.

  // Если будет false - наш код не выведется.
  console.log(message)
}


// Так же, внутри скобок можно использовать и переменные:

let first = 5;
let second = 5;

if (first === second) {
  console.log(message)
}

// А вот и более сложное условие:

if (2 + 1 === 3 && '1' == 1 || 10 > 5 && 10 === 1) {
  console.log(message)
}

// Преобразование типов*:

if ('') {
  console.log(message)
}

// * - Всё что внутри скобок - преобразуется в булев тип, true и false.
// В нашем случае - пустая строка. Она ничего не выведет.
// НО! это же JS - если добавить пробел - всё заработает!

// Короткая запись:

if (2 > 1) console.log(message);

// Итог - если то, что находится внутри круглых скобок выводит true - код внутри
// фигурных скобов будет выполнятся, а если выводит false - не будет выполнятся.

// Блоки ELSE и ELSE IF

// В комментариях выше я написал что код не будет выполнятся если то, что внутри
// обычных скобок - выводит false.
// Но он будет выполнятся, если есть блок else (ещё, иначе) - это то, что будет выводится вместо
// if, если он не выводит true. Т.е - if выводит true а else - false.

// Ещё, кроме if и else есть else if  - "если еще".
// Иногда нужно проверить несколько вариантов условия. Для этого используют
// блок else if. 

message = "Привет, ученик!";
number = 5;

if (number > 10) {
  console.log(message);
} else {
  console.log('Условие не выполнено :(')
}

if (number > 50) {
  console.log('5 больше 50')
} else if (number > 30) {
  console.log('5 больше 30')
} else if (number > 10) {
  console.log('5 больше 10')
} else if (number > 1) {
  console.log('5 больше 1')
} else {
  console.log('Условие не выполнено :(')
}

// Код выше будет проверятся с else if, т.е - если number (5) больше 50 -
// выводи "5 больше 50", а вот если это не так - иди к следующему условию.
// В нашем случае это number (5) > 30: 5 больше 30. И если это так - выводи
// "5 больше 30"! Но.. это тоже неверно. Идём дальше - 5 больше 10, если да - выводи
// "5 больше 10".. но и это неверно! идём дальше - 5 больше 1, если да - выводи
// "5 больше 1"..yes! 5 больше 4, значит последний вариант кода выполнится.
// В противном случае будет выводится то, что находится в else.

// Важно! - блоки else и else if cоответственно являются НЕ обязательными, и
// используются они - довольно редко.

// Условный оператор "?"

message = "Привет";

// 1)

// let messageend;

// if (5 > 1) {
//   messageEnd = ", Вася!";
// } else {
//   messageEnd = ", Оля!";
// }

// 2)
let messageEnd = 5 > 1 ? ", Вася!" : ", Оля!";

message += messageEnd;
console.log(message)

// Первый вариант написания этого кода самый простой, if.. else...
// А вот второй вариант интереснее.
// Мы создали переменную и сразу дали ей значение: 5 больше 1, если да - 
// твое значение будет ", Вася!" (это как с if, если да - то да, нет - то нет.)
// В нашем примере вместо if используется оператор ?, а за else - двоеточие (:).

// И стоит отметить, что оператор ? - тернарный, т.е он "юзает" (от Англ - use)
// сразу 3 операнда.

// С этим оператором можно реализовывать большие и сложные конструкции:

message = "Привет";
messageEnd;

// 1)

// if (5 > 50) {
//   messageEnd = ", Вася!"
// } else if (5 > 30) {
//   messageEnd = ", Оля!"
// } else if (5 > 10) {
//   messageEnd = ", Миша!"
// } else if (5 > 1) {
//   messageEnd = ", Андрей!"
// } else {
//   messageEnd = ", Иннокентий!";
// }

// 2)

messageEnd = 5 > 10 ? ", Вася!" :
	5 > 30 ? ", Оля!" :
		5 > 10 ? ", Миша!" :
			5 > 15 ? ", Андрей!" : ", Иннокентий!"
message += messageEnd;
console.log(message)

// Первый вариант кода нам уже знаком, здесь всё понятно.
// Второй вариант сложнее и менее читабельный, зато короткий, НО! - рекомендуется
// его не использовать, всё также из-за не читабельности.

// Здесь всё тоже самое - if = ?, else = :, else if = (код) ? (код) : (код)

// По той же (опять) причине - ни в коем случае НЕЛЬЗЯ заменять if на ?:

// Не делай так!
5 > 10 ? console.log("Привет, Вася!") :
	console.log("Привет, Оля!");

// Делай так:

if (5 > 10) {
	console.log("Привет, Вася!")
} else {
	console.log("Привет, Оля!")
}

// Его (?) применяют когда надо вывести то, или иное значение.
// Т.е - в коротком коде.

// Домашка

// 1)

if (1 === "1") {
	console.log('Истина!')
} else {
	console.log('Ложь!')
} // F, Ложь!

// 2)

if (5 === '5') {
	console.log('Истина!')
} else {
	console.log('Ложь!')
} // F, Ложь!

// 3)

message = (92 > '11' && 58 < 100) ? 'Истина!' : 'Ложь!';
console.log(message) // T, Истина!

// 4)

if (0) {
	console.log('Ложь!')
} else if (" ") {
	console.log("Истина!")
} // T, Истина!

// Циклы

// Цикл это тот код, который будет повторятся несколько раз и нам не нужно
// его постоянно переписывать, т.е - если нам нужно посчитать от одного до пяти -
// постоянно писать сonsole.log(1), console.log(2) - долго и не красиво.

// Цикл WHILE.

// Синтаксис:

/*
while (Условие) {
  Тело цикла -
  тут будет выполняться код
}
*/

// Лёгкий пример:

let num = 0;
while (num < 5) {
  console.log(num);
  num++;
}

// У нас есть переменная num, у неё есть значение - 0.
// Мы создаём цикл, который будет выполняться тогда, когда значение num - меньше пяти.
// Когда значение num меньше 5 (а оно так и есть, у нас то 0), мы выводим значение
// в консоль (первая строка кода) а потом увеличиваем его, используя инкремент.
// Что случилось? теперь у нас не ноль, а единица.
// Цикл while заметил, что 1 меньше 5, и выполнил код ещё раз - теперь он вывел
// 1 и увеличил его, теперь у нас два - затем он ещё раз так сделал, и ещё и ещё до тех пор,
// пока не дошёл до 4 - ибо 4 меньше 5, а если он выполнит код - 4 станет 5, а
// 5 равно 5.
// Т.е - записываем 0, он true, он меньше 5.
// Увеличиваем 0 на 1, записываем его, он меньше 5 - значит true.
// И так вплоть до 4, это последнее true значение.

// Это можно сравниить с пробежкой спортсмена на улице - он бежит, затем остановка,
// затем ещё раз.. эти остановки - итерации, такие же как у нас в циклах.
// Код выполняется, затем останавливается, затем опять.. и так до того момента, пока
// не дойдет до последнего меньшего значения 5.

// Ещё один пример:

// Пустой console.log( ), не обрашай внимания.
console.log( )

num = 5;
while (num) {
  console.log(num);
  num--;
}

// У нас есть переменная num у которой теперь значение равно пяти.
// Мы создаём цикл, и вписываем в его условие переменную num, без всяких
// выражений и т.д.
// Он (цикл) будет выводить значение переменной а затем уменьшать, и так до тех пор
// пока не дойдет до единицы, ибо 0 = false.

// Вообще, вариантов условий может быть много, море, океаны условий..
// Главное запомнить лишь одно:
/*
"True: бежим
false: не бежим"
(C)
*/
 
// Более того, мы можем сделать цикл ИЗ ОДНОЙ СТРОКИ!

// Пустой сonsole.log( ), не обращай внимания.
console.log( )

num = 10
while(num) console.log(num--);

// Конструкция DO ... WHILE

// Пустой console.log( ), не обращай внимания.
console.log( );

num = 0;
do {
  console.log(num);
  num++;
} while(num < 5)

// Помимо простой записи while есть ещё и запись do ... while. Что значит do?
// do значит "сделать, выполнить", а while - "когда".
// Ну, тут даже объяснять не нужно - по переводу уже всё ясно :)

// Конструкция do ... while уникальна.
// Даже если условие неверно - код будет выводится хотя-бы 1 раз.
// Почему? потому, что тело цикла (то что в скобках у do) выполнится, а затем
// будет проверяться тем, что находится между скобок у while.
// Т.е - сначала выполняется а затем проверяется.

// Но обычно используется обычная запись while, дабы выполнять код лишь
// с удовлетворением условия while - т.е чтобы условие было строго равно true.

// Цикл for

// Синтаксис

/*

for (Начало; Условие; Шаг) {
  тут будет выполняться код.
  Тело цикла,
}

*/

// Пример

// Пустой console.log(), не обращай внимания.
console.log( )

for (i = 10; i < 20; i++) {
  console.log(i)
}

/*
Работа цикла for:

1) Выполняется начало: num = 0, т.е задаём переменной значение

2) Выполняется условие: num < 10

3) Если условие true - выполняется тело цикла - console.log(num)

4) Выполняется шаг - num++

5) Повтор начиная с пунтка №2
*/

// Кстати, та переменная которую мы объявили внутри цикла for: i -
// на самом деле не существует за пределами цикла, за областью видимости. Она
// встроенная, и если обратится к ней при помощи console.log(i) - выдаст ошибку.

// Но мы можем объявить переменную ДО использования цикла, и таким образом 
// использовать её где и как захотим!

// Пустой console.log(), не обращай внимания.
console.log( )

num = 0;
for (num; num < 5; num++) {
  console.log(num)
}

console.log(`Вывод вне цикла: ${num}`)

// На самом деле, мы можем убрать некоторые части цикла for - 
// начало, если мы уже объявили переменную за пределами for
// шаг, мы можем записать его в теле цикла
// и условие тоже, но без условия наш цикл будет идти и идти пока браузер не подохнет от нагрузки.

// Директива break

// Пустой console.log(), не обращай внимания.
console.log( )

num = 0;
for(; num < 5; num++) {
  console.log(num)
  if (num == 2) break;
}

console.log(`Работа окончена, num = ${num}`)

// Допустим, что во время работы цикла нам необходимо вывести только цифры
// 0, 1, 2 - для этого надо завершить цикл досрочно... но как?
// Мы создали условие - if (num == 2), оно означает - "Если значение
// переменной num равно 2 - цикл остановится".
// Директива break останавливает весь цикл и выполняет тот код, который находится
// после фигурных скобок.

// Директива continue

// Пустой console.log(), не обращай внимания.
console.log( )

num = 0;
for (; num < 5; num++) {
  if (num == 2) continue;
  console.log(num)
}

// Допустим, что во время работы цикла нам необходимо ПРОПУСТИТЬ какую-то часть
// тела цикла. Для этого существует директива continue.
// В примере выше мы создали условие, оно означает - "если значение переменной
// num равно 2 - пропусти его и перейди к следующей цифре".
// Т.е - вывод нашего цикла таков: 0, 1, 3, 4 - двойки нету, мы её пропустили.

// Директива break/continue не работает с оператором "?".
// След. код выдаст ошибку:

/*
num = 0;
for (; num < 5; num++) {
  (num != 2) ? console.log(num) : continue;
}
*/

// Метки

// При более сложном варианте использования циклов (например, когда один цикл
// внутри другого) хорошо бы иметь способ прерывать или переключаться на след.
// итерацию род. цикла из дочернего.
// Такой способ есть. И это - метки!

// Пустой console.log( ), не обращай внимания.
console.log( );

firstFor: for (let num = 0; num < 2; num++) {
  for (let size = 0; size < 3; size++) {
    if (size == 2) {
      break firstFor;
    }
    console.log(size)
  }
}

// Директива continue тоже работает с метками:

// Пустой console.log( ), не обращай внимания.
console.log( );

SecondFor: for (let num = 0; num < 2; num ++) {
  for (let size = 0; size < 3; size ++) {
    if (size == 2) {
      continue SecondFor;
    }
    console.log(size)
  }
}

// Итоги и домашка

/* 

1) while - проверяет условие перед каждой итерацией

2) do ... while - проверяет условие после каждой итерации

3) for (;;) - проверяет условие перед каждой итерацией, есть возможность задать
доп. настройки.

Чтобы организовать бесконечный цикл - используют конструкцию while (true).
При этом он, как и любой другой цикл - может быть прервар директивой break.

Если на данной итерации цикла делать больше ничего не надо, но полностью прекращать
цикл не следует - используют директиву continue.

Оба этих директивы поддерживают метки, которые ставятся перед циклом.
Метки - единственный способ для break/continue выйти за пределы текущего цикла
и повлиять на работу внешнего.
Помним, что метки не позволяют прыгнуть в любое место кода, в JS вообще нет такой
возможности.

*/

// Домашка

// №1) Выведи в консоль числа от 1 до 5
console.log( )

for (let num = 1; num < 10; num++) {
  if (num == 6) break;
  console.log(num)
}

// №2)
console.log( )

num = 8;
while (num) {
  // последний результат 0, верно?
  console.log(num);
  num--;
}

// Ответ = нет. 0 false - он не выведется. Последнее - 1.

// №3) Переписать на while.
console.log( )

for (let num = 0; num < 3; num++) {
  console.log(`Число: ${num}`)
}

// Решение:
console.log( )

num = 0;
while (num < 3) {
  console.log(`Число: ${num}`)
  num++;
}

// №4) Прекратить работу цикла №1 когда size равна 1
console.log( )

ThirdFor: for (let num = 0; num < 2; num++) {
  for (let size = 0; size < 3; size++) {
    if (size == 1) {
    break ThirdFor;
    }
    console.log(size)
  }
}

// файл стал слишком огромным! я создал второй файл, там и буду продолжать.