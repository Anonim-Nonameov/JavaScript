...
// эту проблему. Этот метод просто берёт и возвращает нам "вычисленные стили",
// что это значит? вычисленные стили это те стили, которые есть у элемента внешне,
// то есть те стили, которые были заданы элементу через CSS-файл. Таким образом
// мы можем получить абсолютно ВСЕ стили элемента, и не важно, есть они в атрибуте
// style или нет. Причём этот метод возвращает не только те стили, которые есть
// в CSS-файле - он возвращает нам абсолютно все стили элемента, даже самые
// базовые, те, которые были заданы самим браузером. Покажу вам использование
...
// Первым делом мы создаём переменную elementStyles (стили элемента - англ), а
// в качетве значения передаём результат работы метода getComputedStyle над
// элементом, который находится в переменной element. Далее мы просто берём
// и полученное значение выводим в консоль, но не просто так - после того, как
// метод getComputedStyle сработал на элементе, был создан отдельный объект,
// он у нас называется CSSStyleDeclaration. А содержит он в себе все те стили,
// которые есть у этого элемента, их может быть огромное количество - их может
// быть сотни, а может быть и тысячи. И т.к мы получаем объект, мы соответственно
// можем получить и значения его свойств! в коде выше мы вывели в консоль значение
// свойства font-size, в результате получили строку "16px". Значит, это свойство
// имеет значение в 16 пикселей. Так, про это узнали, идём дальше!

// getComputedStyle - использование метода с псевдоэлементами

// Помимо возвращения общих стилей у элемента, этот метод использоваться также
// и для того, чтобы возвращать стили псевдоэлемента! для этого, нам необходимо
// после указания элемента, с которым будем работать метод, необходимо через
// запятую указать псевдоэлемент. Посмотрим на примере, но перед этим мы должны
// изменить значение переменной element на другое, ибо использоваться будет
// другой элемент, который имеет при себе псевдоэлемент:

element = document.querySelector(".lesson__list-item_what"); 

let elementAfterStyles = getComputedStyle(element, "::after");
console.log(elementAfterStyles.fontStyle);
// => "italic"

// Первым делом мы меняем значение переменной element на другое, поскольку нам
// необходим другой элемент для работы. После этого, мы создаём другую переменную,
// называется она elementAfterStyles, а в качестве значение мы ей присваиваем
// результат работы методы getComputedStyle над псевдоэлементом элемента, который
// хранится в переменной element, как бы странно это ни звучало. В результате мы
// получаем объект CSSStyleDeclaration, а дальше мы просто берём и выводим
// значение ключа "fontStyle" этого же объекта в консоль, после чего мы получаем
// строку "italic", что означает, что значение свойства font-size - italic.
// Прекрасно, идём дальше!

// getComputedStyle - уверенность в получении значения или "ЧЕГО ТЫ ХОЧЕШЬ?!"

// Итак, следует добавить, что если мы с вами хотим получить какое-то конкретное
// значение свойства, то стоит писать его ТОЧНОЕ и ПОЛНОЕ название. Покажу
// это на примере:

elementStyles = getComputedStyle(element);
console.log(elementStyles.paddingLeft);
// => 10px

// Мы хотим получить внутренний отступ слева. На примере выше мы применяем метод
// getComputedStyle на элементе element, далее мы выводим в консоль значение
// свойства padding-left, хранимого в объекте, который мы получили в результате
// работы методы. Далее, мы получаем в консоль запись "10px", и это значит, что у
// свойства padding-left значение в 10 пикселей. Но в данном случае мы взяли и
// явно указали то свойство, значение которого нам хотелось бы узнать, а именно
// то свойство, которое отвечает за внутренний отступ слева. Что будет, если мы
// НЕ укажем это свойство, а именно padding-left, а просто общее свойство padding?

console.log(elementStyles.padding);
// => 0px 0px 0px 10px / FF - "" (empty string)

// В таком случае, мы получаем полную запись свойства, состоящую из 4-ёх значений:
// отступ сверху, справа, снизу и СЛЕВА. И да - эту общую запись мы увидим только
// в браузере Google Chrome, в Firefox мы вообще ничего не получим, просто пустую
// строку. Идём дальше!

// getComputedStyle - "Не трожь!" или режим "только для чтения"

// Работа со значениями CSS-свойств с помощью метода getComputedStyle работает
// только для чтения. Что это значит? а значит, что все те свойства, хранимые
// в объекте, который создаёт этот метод, недоступны для изменений, то есть
// их значения невозможно как-то изменить. Покажу на примере:

// elementStyles.paddingLeft = "25px";
// ---
// => Uncaught DOMException: Failed to set the 'padding-left' property on
// 'CSSStyleDeclaration': These styles are computed, and therefore the
// 'padding-left' property is read-only.

// Итак, в примере выше мы пытаемся изменить значение свойства padding-left
// его текущего на другое, но у нас не получается и в консоли выдаёт ошибку (и
// именно по этой причине я закомментировал код на 974-ой строке), но что именно
// говорится в этой ошибке? говорится, что свойства, хранимые в объекте, который
// создаёт метод getComputedStyle - это вычисленные стили, и они доступны только
// для чтения, их невозможно изменить.

// Стили и классы: облегчи себе жизнь, или "лайфхаки"

// Лайфхак первый

// С практикой мы закончили, осталось лишь рассказать вам о лайфхаках, связанными
// со стилями. Начнём с getComputedStyle. В целом, мы используем метод gTS для
// того, чтобы получить текущее значение того или иного свойства. Например,
// получить текущую ширину для того, чтобы добавить к ней что-то, отнять от
// неё что-то, изменить, одним словом - нам нужно число. Но, когда мы берём
// и пытаемся получить значение какого нибудь свойства, которое содержит в себе
// и число и строку, то бишь "число + px", мы получаем как раз СТРОКУ:

elementStyles = getComputedStyle(element);
console.log(elementStyles.paddingLeft);
// => 10px
// Тысячная строка, йеее! :D

// В примере выше мы применяем метод getComputedStyle на элементе element, в
// результате мы получаем объект, содержащий в себе стили элемента, и выводим
// в консоль одно из значений его свойств (стилей), а именно padding-left.
// По итогу, в консоли появляется СТРОКА "10px", то есть число + пиксели,
// которые нам не нужны (ну.. почти :D), а мы знаем, что если в значении
// есть хоть одна буква, то это - строка, не число. Но, исправить сложившуюся
// ситуацию можно используя метод parseInt, который мы изучали в одном из уроков.
// Он берёт значение, к которому его применяют и превращает его в число. Если во
// время чтения значения метод обнаруживает число, а после него строку, что
// несомненно приведет к ошибке, поскольку Number + String = NaN, то он берёт
// и возвращает то число, которое было получено до ошибки. Покажу его работу
// на примере:

let paddingLeft = parseInt(elementStyles.paddingLeft);
console.log(paddingLeft);
// => 10 (число)

// Какого-то особого объяснения код выше не требует - мы создаём переменную
// paddingLeft, и в качестве значения передаем результат работы метода parseInt
// над значением свойства padding-left, которое хранится в объекте, который
// в свою очередь содержится в переменной elementStyles. Далее мы выводим в
// консоль значение переменной paddingLeft, в результате в консоли получаем
// число 10. Метод parseInt вернул только число, а строку "px" он убрал.

// Лайфхак второй

// Второй лайфхак даже не лайфхак, а скорее напоминание о том, что нужно помнить
// про единицы измерения, когда мы хотим присвоить то или иное значение какому-то
// стилевому свойству. Например, взглянём на пример снизу:

element.style.marginLeft = 20;

// В данном случае мы берём и устанавливаем свойству margin-left значение в 20..
// но не чего-то, а просто 20. По итогу у нас ничего не сработает, так как это
// недопустимое значение, и в атрибуте style у элемента ничего не появится.

// Файл стал слишком большим, поэтому я создал новый :)